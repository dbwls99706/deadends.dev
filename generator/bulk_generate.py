...
def canon(
    ...
    return {
        ...
        "metadata": {
            ...
            "evidence_count": sum(len(d.get("sources", [])) for d in dead_ends) + sum(len(w.get("sources", [])) for w in workarounds),
...
def get_all_canons() -> list[dict]:
    ...
    canons.append(canon(
        ...
        [de("Retry immediately in a tight loop", "Hammering a 429 endpoint causes progressive penalty escalation",
            0.95,
            sources=["https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/429"]),
         de("Only add a fixed 1-second delay", "Fixed delay ignores the Retry-After value",
            0.70,
            sources=["https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Retry-After"])],
        [wa("Parse Retry-After header and sleep for the specified duration", 0.93,
            "const retryAfter = parseInt(res.headers['retry-after'], 10) || 1; await sleep(retryAfter * 1000);",
            sources=["https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Retry-After"]),
         wa("Implement exponential backoff with jitter", 0.90,
    ...
    canons.append(canon(
        ...
        [de("Force decode as ASCII", "ASCII cannot represent multi-byte characters",
            0.90,
            sources=["https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Type"]),
         de("Add charset to Accept header expecting server to echo it", "Most APIs ignore charset in Accept header",
            0.75,
            sources=["https://www.rfc-editor.org/rfc/rfc7231#section-5.3.2"])],
        [wa("Explicitly decode response body as UTF-8 regardless of header", 0.92,
            "response.content.decode('utf-8')",
            sources=["https://docs.python-requests.org/en/latest/user/advanced/#encodings"]),
         wa("Set response.encoding before accessing .text", 0.90,
    ...
    canons.append(canon(
        ...
        [de("Rely on client-side deduplication only", "Client cannot prevent server-side duplicates if the server never sees the idempotency key",
            0.80,
            sources=["https://stripe.com/docs/api/idempotent_requests"]),
         de("Use request body hash as idempotency key", "Body hash changes if any field differs slightly",
            0.70,
            sources=["https://brandur.org/idempotency-keys"])],
        [wa("Ensure proxy passes Idempotency-Key header through", 0.93,
            "nginx: proxy_pass_header Idempotency-Key;",
            sources=["https://stripe.com/docs/api/idempotent_requests"]),
         wa("Generate UUID-based idempotency key at the outermost client layer", 0.88,
    ...
    canons.append(canon(
        ...
        [de("Strip W/ prefix before comparing", "Stripping W/ converts a weak ETag to strong; they have different semantics per RFC 7232",
            0.85,
            sources=["https://www.rfc-editor.org/rfc/rfc7232#section-2.1"]),
         de("Use If-Match instead of If-None-Match with weak ETags", "If-Match requires strong comparison by spec; weak ETags will always fail",
            0.90,
            sources=["https://www.rfc-editor.org/rfc/rfc7232#section-3.1"])],
        [wa("Use If-None-Match with weak comparison as specified in RFC 7232", 0.88,
            "Pass the full W/ value as-is",
            sources=["https://www.rfc-editor.org/rfc/rfc7232#section-3.2"]),
         wa("Request strong ETag from server via Cache-Control or API option", 0.80,
    ...
    canons.append(canon(
        ...
        [de("Manually set Content-Type: multipart/form-data header", "When you set Content-Type manually, the boundary will not match what the HTTP library generates",
            0.92,
            sources=["https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Type"]),
         de("Copy boundary from a previous request", "Each multipart request generates a unique boundary; reusing old ones causes mismatch",
            0.85,
            sources=["https://www.rfc-editor.org/rfc/rfc2046#section-5.1"])],
        [wa("Let the HTTP library set Content-Type automatically for multipart", 0.95,
         ...
         wa("If manual boundary is needed, ensure exact match between header and body", 0.85,
            "boundary='myboundary'; headers={'Content-Type': f'multipart/form-data; boundary={boundary}'}",
            sources=["https://www.rfc-editor.org/rfc/rfc2046#section-5.1"])],
    ))

    # =====================================================================
    # === CLOUD EXPANSION (5 extra) ===
    # =====================================================================
    canons.append(canon(
        ...
        [de("Increase OS TCP timeout beyond 350s", "NAT Gateway drops the mapping regardless of OS settings",
            0.90,
            sources=["https://docs.aws.amazon.com/vpc/latest/userguide/nat-gateway-troubleshooting.html"]),
         de("Use longer connection pool idle timeout", "Application-level idle timeout does not prevent NAT Gateway from dropping the mapping",
            0.80,
            sources=["https://docs.aws.amazon.com/vpc/latest/userguide/nat-gateway-troubleshooting.html"])],
        [wa("Enable TCP keepalive with interval under 350 seconds", 0.92,
            "sysctl -w net.ipv4.tcp_keepalive_time=120",
            sources=["https://docs.aws.amazon.com/vpc/latest/userguide/nat-gateway-troubleshooting.html"]),
         wa("Use VPC endpoints to bypass NAT Gateway entirely", 0.88,
    ...
    canons.append(canon(
        ...
        [de("Add metadata.google.internal to /etc/hosts", "The IP is already 169.254.169.254; hosts file does not fix network-level issues",
            0.75,
            sources=["https://cloud.google.com/compute/docs/metadata/overview"]),
         de("Increase HTTP client timeout to 60s", "Longer timeout alone does not help; need retry logic",
            0.65,
            sources=["https://cloud.google.com/compute/docs/metadata/overview"])],
        [wa("Implement retry with exponential backoff on metadata requests", 0.90,
            "for i in range(5): try: return requests.get(url, timeout=5) except: time.sleep(2**i)",
            sources=["https://cloud.google.com/compute/docs/metadata/overview"]),
         wa("Use Metadata-Flavor: Google header and retry on 503", 0.88,
            "headers={'Metadata-Flavor': 'Google'}",
            sources=["https://cloud.google.com/compute/docs/metadata/querying-metadata"])],
    ))
    canons.append(canon(
        ...
        [de("Force delete the blob without breaking lease first", "Delete fails with 409 Conflict when a lease is active",
            0.92,
            sources=["https://learn.microsoft.com/en-us/rest/api/storageservices/lease-blob"]),
         de("Wait for lease to expire naturally", "Infinite leases never expire; even finite leases can be 60s",
            0.70,
            sources=["https://learn.microsoft.com/en-us/rest/api/storageservices/lease-blob"])],
        [wa("Break the lease before modifying or deleting the blob", 0.93,
            "blob_client.break_lease()",
            sources=["https://learn.microsoft.com/en-us/rest/api/storageservices/lease-blob"]),
         wa("Include x-ms-lease-id header if you hold the lease", 0.90,
    ...
    canons.append(canon(
        ...
        [de("Manually increase storage during read-only state", "Manual scaling is also subject to the 6-hour cooldown",
            0.60,
            sources=["https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_PIOPS.StorageTypes.html"]),
         de("Restart the RDS instance to clear read-only", "Restart does not free storage; instance goes read-only again immediately",
            0.85,
            sources=["https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/Overview.DBInstance.Status.html"])],
        [wa("Set generous max storage threshold for auto-scaling with CloudWatch alarm at 80 percent", 0.90,
            "aws rds modify-db-instance --max-allocated-storage 500 --db-instance-identifier mydb",
            sources=["https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_PIOPS.StorageTypes.html"]),
         wa("Purge old data or archive to S3 to free space", 0.85,
    ...
    canons.append(canon(
        ...
        [de("Set visibility timeout equal to average processing time", "Average is not max; longer processing will cause reprocessing",
            0.80,
            sources=["https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-visibility-timeout.html"]),
         de("Delete message before processing completes", "If processing fails after delete, the message is lost forever",
            0.85,
            sources=["https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-visibility-timeout.html"])],
        [wa("Set visibility timeout to 6x max processing time and extend with ChangeMessageVisibility", 0.93,
            "sqs.change_message_visibility(QueueUrl=q, ReceiptHandle=rh, VisibilityTimeout=300)",
            sources=["https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-visibility-timeout.html"]),
         wa("Make processing idempotent so reprocessing has no side effects", 0.90,
            "Use a deduplication key and check before processing",
            sources=["https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/using-messagededuplicationid-property.html"])],
    ))

    # =====================================================================
    # === LLM EXPANSION (5 extra) ===
    # =====================================================================
    canons.append(canon(
        ...
        [de("Filter exact phrase ignore previous instructions", "Trivially bypassed by rephrasing, unicode tricks, or multilingual variants",
            0.85,
            sources=["https://owasp.org/www-project-top-10-for-large-language-model-applications/"]),
         de("Put system prompt at the end of context", "LLMs attend to all positions; placement alone does not prevent extraction",
            0.70,
            sources=["https://simonwillison.net/2023/Apr/14/worst-that-can-happen/"])],
        [wa("Use a separate system message role and enforce input/output guardrails", 0.75,
         ...
         wa("Implement layered defense: input sanitization + output filtering + canary tokens", 0.70,
    ...
    canons.append(canon(
        ...
        [de("Just increase max_tokens parameter", "max_tokens controls output length, not input",
            0.90,
            sources=["https://platform.openai.com/docs/api-reference/chat/create"]),
         de("Split input into multiple sequential API calls", "Sequential calls do not share context",
            0.70,
            sources=["https://platform.openai.com/docs/guides/rate-limits"])],
        [wa("Count tokens with tiktoken before sending and chunk within limits", 0.92,
            "import tiktoken; enc = tiktoken.encoding_for_model('gpt-4'); len(enc.encode(text))",
            sources=["https://github.com/openai/tiktoken"]),
         wa("Use a sliding window or map-reduce pattern for long documents", 0.85,
    ...
    canons.append(canon(
        ...
        [de("Set a universal threshold like 0.8 for all embedding models", "Different models have different similarity distributions",
            0.85,
            sources=["https://www.sbert.net/docs/sentence_transformer/usage/semantic_textual_similarity.html"]),
         de("Use Euclidean distance instead of cosine for normalized embeddings", "For normalized vectors, cosine and Euclidean are monotonically related",
            0.70,
            sources=["https://www.pinecone.io/learn/vector-similarity/"])],
        [wa("Calibrate threshold per embedding model using a validation set", 0.88,
            "Find the threshold that maximizes F1 score on known-relevant docs",
            sources=["https://www.pinecone.io/learn/vector-similarity/"]),
         wa("Use top-K retrieval instead of threshold-based filtering", 0.90,
    ...
    canons.append(canon(
        ...
        [de("Execute all parallel tool calls concurrently as requested", "Concurrent execution of dependent tool calls causes race conditions",
            0.75,
            sources=["https://platform.openai.com/docs/guides/function-calling"]),
         de("Ignore the parallel flag and run sequentially always", "Running everything sequentially defeats the performance benefit",
            0.60,
            sources=["https://platform.openai.com/docs/guides/function-calling"])],
        [wa("Detect resource dependencies and serialize conflicting tool calls", 0.85,
            "Group tool calls by target resource; run independent groups in parallel",
            sources=["https://platform.openai.com/docs/guides/function-calling"]),
         wa("Make tool implementations idempotent and conflict-safe", 0.82,
    ...
    canons.append(canon(
        ...
        [de("JSON.parse each chunk individually", "Stream chunks are arbitrary byte boundaries, not JSON boundaries",
            0.95,
            sources=["https://platform.openai.com/docs/api-reference/streaming"]),
         de("Wait for a closing brace to trigger parsing", "Nested objects have multiple closing braces; triggering on first } gives incomplete parse",
            0.75,
            sources=["https://platform.openai.com/docs/api-reference/streaming"])],
        [wa("Accumulate stream chunks into a buffer and parse only when complete", 0.93,
            "buffer += chunk; try: result = json.loads(buffer) except: continue",
            sources=["https://platform.openai.com/docs/api-reference/streaming"]),
         wa("Use a streaming JSON parser that handles partial input", 0.88,
            "Use libraries like partial-json-parser or ijson",
            sources=["https://github.com/promplate/partial-json-parser"])],
    ))

    # =====================================================================
    # === DATA EXPANSION (5 extra) ===
    # =====================================================================
    canons.append(canon(
        ...
        [de("Force cast all columns to string", "Casting to string destroys type information and breaks downstream numeric operations",
            0.80,
            sources=["https://arrow.apache.org/docs/python/parquet.html"]),
         de("Use schema from first file for all files", "First file schema may be outdated; newer files with added columns will lose data",
            0.70,
            sources=["https://arrow.apache.org/docs/python/parquet.html"])],
        [wa("Use unify_schemas to create a merged schema", 0.90,
            "pa.unify_schemas([f.schema_arrow for f in files])",
            sources=["https://arrow.apache.org/docs/python/parquet.html"]),
         wa("Read with pyarrow dataset API which handles schema evolution natively", 0.88,
    ...
    canons.append(canon(
        ...
        [de("Delete and re-register the schema", "Deleting a schema does not remove it from consumer caches",
            0.90,
            sources=["https://docs.confluent.io/platform/current/schema-registry/fundamentals/schema-evolution.html"]),
         de("Change compatibility mode to NONE", "NONE mode disables protection; consumers will crash on incompatible messages",
            0.80,
            sources=["https://docs.confluent.io/platform/current/schema-registry/fundamentals/schema-evolution.html"])],
        [wa("Add new fields with default values to maintain backward compatibility", 0.92,
            '{"name": "new_field", "type": "string", "default": ""}',
            sources=["https://docs.confluent.io/platform/current/schema-registry/fundamentals/schema-evolution.html"]),
         wa("Use FULL_TRANSITIVE compatibility and test schemas before registration", 0.85,
            "confluent schema-registry compatibility --check --subject mysubject --schema new.avsc",
            sources=["https://docs.confluent.io/platform/current/schema-registry/fundamentals/schema-evolution.html"])],
    ))
    canons.append(canon(
        ...
        [de("Just delete the field and reuse the number", "Old serialized data still uses that field number",
            0.95,
            sources=["https://protobuf.dev/programming-guides/proto3/#reserved"]),
         de("Rename the field but keep the same number", "Field names are ignored in wire format; only the number matters",
            0.60,
            sources=["https://protobuf.dev/programming-guides/encoding/"])],
        [wa("Mark deleted field numbers as reserved", 0.95,
            'message Foo { reserved 2, 15; reserved "old_field"; }',
            sources=["https://protobuf.dev/programming-guides/proto3/#reserved"]),
         wa("Always use the next available field number for new fields", 0.90,
            "Never reuse field numbers. Keep a CHANGELOG of assignments",
            sources=["https://protobuf.dev/programming-guides/proto3/#assigning"])],
    ))
    canons.append(canon(
        ...
        [de("Assume all msgpack libraries support timestamp extension type", "Older or minimal implementations may not support the timestamp extension",
            0.85,
            sources=["https://github.com/msgpack/msgpack/blob/master/spec.md#timestamp-extension-type"]),
         de("Convert timestamps to strings before packing", "String timestamps are larger and defeat msgpack binary efficiency",
            0.65,
            sources=["https://github.com/msgpack/msgpack/blob/master/spec.md"])],
        [wa("Serialize timestamps as Unix epoch integers", 0.92,
            "int(datetime.now().timestamp()) -- universally supported",
            sources=["https://github.com/msgpack/msgpack/blob/master/spec.md"]),
         wa("Use explicit extension type registration on both sides", 0.82,
    ...
    canons.append(canon(
        ...
        [de("Force little-endian byte order in application code", "Arrow IPC uses native endianness; application-level byte swapping does not affect IPC format",
            0.80,
            sources=["https://arrow.apache.org/docs/format/Columnar.html"]),
         de("Use Arrow Flight instead of IPC files", "Arrow Flight also uses native endianness",
            0.60,
            sources=["https://arrow.apache.org/docs/format/Flight.html"])],
        [wa("Standardize on little-endian and convert during write on BE systems", 0.85,
            "Most modern systems are little-endian; convert at write time on BE",
            sources=["https://arrow.apache.org/docs/format/Columnar.html"]),
         wa("Use Parquet format instead of IPC for cross-platform exchange", 0.90,
            "Parquet has defined endianness (little-endian); portable across all architectures",
            sources=["https://parquet.apache.org/documentation/latest/"])],
    ))

    # =====================================================================
    # === SECURITY EXPANSION (5 extra) ===
    # =====================================================================
    canons.append(canon(
        ...
        [de("Add unsafe-inline to fix CSP errors from inline scripts", "unsafe-inline allows ANY inline script, completely defeating XSS protection",
            0.92,
            sources=["https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/script-src"]),
         de("Use both nonce and unsafe-inline together for compatibility", "Browsers supporting nonces ignore unsafe-inline; those not supporting nonces allow all inline",
            0.70,
            sources=["https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/script-src"])],
        [wa("Use nonce-based CSP and add nonce attribute to all legitimate inline scripts", 0.93,
            "script-src 'nonce-abc123'",
            sources=["https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/script-src"]),
         wa("Move all inline scripts to external files and use strict-dynamic", 0.88,
    ...
    canons.append(canon(
        ...
        [de("Escape user input with custom string replacement", "Custom escaping always misses edge cases",
            0.90,
            sources=["https://owasp.org/www-community/attacks/SQL_Injection"]),
         de("Use a WAF to block SQL injection patterns", "WAFs can be bypassed with encoding tricks",
            0.70,
            sources=["https://owasp.org/www-community/attacks/SQL_Injection"])],
        [wa("Always use parameterized queries, even in ORM raw mode", 0.95,
            "Model.objects.raw('SELECT * FROM t WHERE id = %s', [user_id])",
            sources=["https://docs.djangoproject.com/en/5.0/topics/db/sql/#passing-parameters-into-raw"]),
         wa("Use ORM query builder methods instead of raw SQL", 0.90,
    ...
    canons.append(canon(
        ...
        [de("Store token in localStorage instead", "localStorage is always accessible to JavaScript; even more vulnerable to XSS",
            0.85,
            sources=["https://owasp.org/www-community/HttpOnly"]),
         de("Add XSS filtering in JavaScript to protect cookies", "Client-side filtering can be bypassed; attacker controls the JavaScript context",
            0.80,
            sources=["https://owasp.org/www-community/HttpOnly"])],
        [wa("Set HttpOnly and Secure flags on all session cookies", 0.95,
            "Set-Cookie: session=abc123; HttpOnly; Secure; SameSite=Strict",
            sources=["https://owasp.org/www-community/HttpOnly"]),
         wa("Use SameSite=Strict alongside HttpOnly", 0.90,
    ...
    canons.append(canon(
        ...
        [de("Filter out ../ with string replace", "Attackers use URL encoding, double encoding, or OS-specific separators to bypass",
            0.90,
            sources=["https://owasp.org/www-community/attacks/Path_Traversal"]),
         de("Check if path starts with allowed directory using startswith", "Path may resolve differently after symlink resolution",
            0.80,
            sources=["https://owasp.org/www-community/attacks/Path_Traversal"])],
        [wa("Resolve the full canonical path and verify it is within the allowed directory", 0.93,
            "real = os.path.realpath(os.path.join(base, user_input)); assert real.startswith(os.path.realpath(base))",
            sources=["https://owasp.org/www-community/attacks/Path_Traversal"]),
         wa("Use a whitelist of allowed filenames", 0.90,
    ...
    canons.append(canon(
        ...
        [de("Validate the serialized data before deserializing", "The exploit triggers during deserialization itself, not after",
            0.90,
            sources=["https://owasp.org/www-project-top-ten/2017/A8_2017-Insecure_Deserialization"]),
         de("Use a custom ObjectInputStream with class whitelist alone", "Gadget chains can use allowed classes to achieve code execution",
            0.65,
            sources=["https://owasp.org/www-project-top-ten/2017/A8_2017-Insecure_Deserialization"])],
        [wa("Replace unsafe deserializers with safe formats like JSON or Protobuf", 0.95,
            "json.loads(data) instead of pickle.loads(data)",
            sources=["https://docs.python.org/3/library/pickle.html#restricting-globals"]),
         wa("If pickle is required, use RestrictedUnpickler with strict class allowlist", 0.78,
            "class SafeUnpickler(pickle.Unpickler): def find_class(self, module, name): ...",
            sources=["https://docs.python.org/3/library/pickle.html#restricting-globals"])],
    ))

    # =====================================================================
    # === POLICY EXPANSION (5 extra) ===
    # =====================================================================
    canons.append(canon(
        ...
        [de("Buy more bandwidth packs repeatedly", "Bandwidth packs are one-time; recurring large files get expensive fast",
            0.60,
            sources=["https://docs.github.com/en/billing/managing-billing-for-git-large-file-storage"]),
         de("Remove LFS tracking to avoid bandwidth limits", "Removing tracking does not remove existing LFS objects",
            0.80,
            sources=["https://docs.github.com/en/repositories/working-with-files/managing-large-files/about-git-large-file-storage"])],
        [wa("Migrate large files to external storage (S3) and store URLs instead", 0.90,
            "Store large assets in S3 with presigned URLs",
            sources=["https://docs.github.com/en/repositories/working-with-files/managing-large-files/about-git-large-file-storage"]),
         wa("Use git lfs prune and audit .gitattributes", 0.82,
    ...
    canons.append(canon(
        ...
        [de("Compress the package more aggressively", "Most dependencies are already compressed efficiently",
            0.70,
            sources=["https://docs.aws.amazon.com/lambda/latest/dg/gettingstarted-limits.html"]),
         de("Split into multiple Lambda functions", "If the oversized component is a shared dependency, splitting does not help",
            0.65,
            sources=["https://docs.aws.amazon.com/lambda/latest/dg/gettingstarted-limits.html"])],
        [wa("Use Lambda layers for shared dependencies", 0.88,
         ...
         wa("Use container image packaging (up to 10GB)", 0.92,
    ...
    canons.append(canon(
        ...
        [de("Tree-shake imports to reduce bundle size", "Some libraries are not tree-shakeable",
            0.60,
            sources=["https://vercel.com/docs/functions/edge-functions/limitations"]),
         de("Minify the code more aggressively", "Minification has diminishing returns; large dependencies are the problem",
            0.55,
            sources=["https://vercel.com/docs/functions/edge-functions/limitations"])],
        [wa("Move heavy dependencies to serverless functions and call from edge", 0.90,
            "Edge function handles routing; calls serverless for heavy work",
            sources=["https://vercel.com/docs/functions/edge-functions/limitations"]),
         wa("Use dynamic imports and code splitting", 0.82,
    ...
    canons.append(canon(
        ...
        [de("Always use the latest API version without pinning", "Unpinned version auto-upgrades when Stripe updates; webhook format changes break handlers",
            0.85,
            sources=["https://stripe.com/docs/api/versioning"]),
         de("Only pin version in code but not in webhook endpoint settings", "Webhook events use the endpoint API version, not the code version",
            0.75,
            sources=["https://stripe.com/docs/webhooks/api-versions"])],
        [wa("Pin API version in both SDK and webhook endpoint configuration", 0.93,
            "stripe.api_version = '2023-10-16'  # also pin in Dashboard > Webhooks",
            sources=["https://stripe.com/docs/api/versioning"]),
         wa("Test webhook payloads with Stripe CLI before upgrading", 0.85,
    ...
    canons.append(canon(
        ...
        [de("Add more Page Rules to handle all edge cases", "Free plan is limited to 3 Page Rules",
            0.90,
            sources=["https://developers.cloudflare.com/rules/page-rules/"]),
         de("Use Workers to supplement Page Rules on free plan", "Free Workers plan has 100K requests/day limit",
            0.60,
            sources=["https://developers.cloudflare.com/workers/platform/limits/"])],
        [wa("Upgrade to Pro plan for 20 Page Rules and rule ordering", 0.85,
            "$20/month Pro plan includes 20 Page Rules with explicit priority",
            sources=["https://developers.cloudflare.com/rules/page-rules/"]),
         wa("Consolidate rules using a single Worker with explicit routing logic", 0.80,
    ...
    # =====================================================================
    # === COMMUNICATION EXPANSION (5 extra) ===
    # =====================================================================
    canons.append(canon(
        ...
        [de("Disable signature validation as a workaround", "Disabling validation exposes the webhook to spoofed requests",
            0.95,
            sources=["https://www.twilio.com/docs/usage/webhooks/webhooks-security"]),
         de("Hardcode the webhook URL in validation", "Hardcoded URL becomes wrong when deployment URL changes",
            0.65,
            sources=["https://www.twilio.com/docs/usage/webhooks/webhooks-security"])],
        [wa("Use X-Forwarded-Proto and X-Forwarded-Host to reconstruct the original URL", 0.90,
            "Reconstruct URL from forwarded headers for signature validation",
            sources=["https://www.twilio.com/docs/usage/webhooks/webhooks-security"]),
         wa("Configure Twilio webhook URL to match the exact public-facing URL", 0.88,
    ...
    canons.append(canon(
        ...
        [de("Use port 465 with STARTTLS", "Port 465 expects implicit TLS; STARTTLS is an upgrade from plaintext",
            0.95,
            sources=["https://www.rfc-editor.org/rfc/rfc8314"]),
         de("Use port 587 with implicit SSL/TLS", "Port 587 starts plaintext and upgrades via STARTTLS",
            0.95,
            sources=["https://www.rfc-editor.org/rfc/rfc8314"])],
        [wa("Use port 587 with STARTTLS for submission (RFC 8314)", 0.93,
            "smtplib.SMTP('smtp.example.com', 587); server.starttls()",
            sources=["https://www.rfc-editor.org/rfc/rfc8314"]),
         wa("Use port 465 with implicit TLS when STARTTLS is not supported", 0.88,
    ...
    canons.append(canon(
        ...
        [de("Increase Sentry plan quota blindly", "The root cause is often noisy errors flooding the quota",
            0.60,
            sources=["https://docs.sentry.io/product/accounts/quotas/"]),
         de("Disable rate limiting on the Sentry project", "Without rate limits, a single noisy error can consume the entire org quota",
            0.75,
            sources=["https://docs.sentry.io/product/accounts/quotas/manage-event-stream-guide/"])],
        [wa("Configure beforeSend to filter out noisy, low-value errors", 0.92,
         ...
         wa("Set per-key rate limits and use inbound filters", 0.88,
    ...
    canons.append(canon(
        ...
        [de("Just remove all tags from metrics", "Removing all tags makes metrics ungroupable and useless for debugging",
            0.80,
            sources=["https://docs.datadoghq.com/metrics/custom_metrics/"]),
         de("Switch to logs for everything instead of metrics", "Log ingestion at high volume is often more expensive than metrics",
            0.70,
            sources=["https://docs.datadoghq.com/metrics/custom_metrics/"])],
        [wa("Audit tags for cardinality: remove unbounded tags like user_id", 0.92,
            "Replace user_id with user_tier in Metrics Summary",
            sources=["https://docs.datadoghq.com/metrics/custom_metrics/"]),
         wa("Use Metrics without Limits to control indexing", 0.85,
            "Configure tag allowlist: only index low-cardinality tags",
            sources=["https://docs.datadoghq.com/metrics/metrics-without-limits/"])],
    ))
    canons.append(canon(
        ...
        [de("Disable alert grouping entirely", "Without grouping, alert storms create hundreds of incidents",
            0.70,
            sources=["https://support.pagerduty.com/docs/intelligent-alert-grouping"]),
         de("Set grouping window to very short (1 minute)", "Short windows still group rapid-fire alerts",
            0.65,
            sources=["https://support.pagerduty.com/docs/intelligent-alert-grouping"])],
        [wa("Use content-based alert grouping with specific dedup_key patterns", 0.88,
            "Set dedup_key to include service + error_type",
            sources=["https://support.pagerduty.com/docs/intelligent-alert-grouping"]),
         wa("Configure time-based grouping with appropriate window", 0.82,
            "Set 5-minute window; review grouped incidents weekly to tune",
            sources=["https://support.pagerduty.com/docs/intelligent-alert-grouping"])],
    ))

    # =====================================================================
    # === PATH QUIRKS: Korean/CJK, Spaces, Hyphens in paths/folders ===
    # =====================================================================
    ...
    canons.append(canon(
        ...
        [de("Rename the folder to ASCII", "Works but does not fix the root cause; other Korean paths will still break",
            0.50,
            sources=["https://docs.python.org/3/howto/unicode.html"]),
         de("Set PYTHONIOENCODING=utf-8 only", "PYTHONIOENCODING affects stdout/stderr encoding, not filesystem path encoding",
            0.70,
            sources=["https://docs.python.org/3/using/cmdline.html#envvar-PYTHONIOENCODING"])],
        [wa("Set locale to UTF-8: export LANG=en_US.UTF-8 (or ko_KR.UTF-8) and ensure locale-gen is run", 0.95,
            "export LANG=ko_KR.UTF-8; sudo locale-gen ko_KR.UTF-8  # fixes filesystem encoding",
            sources=["https://docs.python.org/3/howto/unicode.html"]),
         wa("Set PYTHONUTF8=1 environment variable (Python 3.7+ UTF-8 mode)", 0.92,
    ...
    canons.append(canon(
        ...
        [de("Downgrade npm version", "Path encoding handling is an OS/filesystem issue, not npm-version-specific",
            0.80,
            sources=["https://docs.npmjs.com/common-errors"]),
         de("Use --legacy-peer-deps flag", "Flag is for dependency resolution conflicts, not path encoding issues",
            0.90,
            sources=["https://docs.npmjs.com/cli/v10/commands/npm-install"])],
        [wa("Move project to an ASCII-only path or create a symlink from an ASCII path", 0.92,
            "ln -s /home/user/프로젝트 /home/user/project && cd /home/user/project && npm install",
            sources=["https://docs.npmjs.com/common-errors"]),
         wa("Set system locale to UTF-8 and use Node.js with --icu-data-dir for full ICU support", 0.88,
    ...
    canons.append(canon(
        ...
        [de("Escape the Korean characters in the docker build command", "Docker CLI does not support escaped Unicode paths in -f flag",
            0.90,
            sources=["https://docs.docker.com/engine/reference/commandline/build/"]),
         de("Use DOCKER_BUILDKIT=0 to fall back to legacy builder", "Legacy builder has the same path encoding limitation",
            0.80,
            sources=["https://docs.docker.com/build/buildkit/"])],
        [wa("Run docker build from an ASCII-only path using absolute Dockerfile path", 0.92,
            "cp Dockerfile /tmp/build/ && docker build -f /tmp/build/Dockerfile .",
            sources=["https://docs.docker.com/engine/reference/commandline/build/"]),
         wa("Create a symlink with ASCII name pointing to the Korean-named directory", 0.88,
            "ln -s /home/유저/프로젝트 /tmp/project && docker build /tmp/project",
            sources=["https://docs.docker.com/engine/reference/commandline/build/"])],
    ))

    canons.append(canon(
        "git", "korean-filename-escaped-display", "git2-any",
        "git status shows Korean filenames as escaped octal: \\354\\225\\210\\353",
        r"\\[0-9]{3}\\[0-9]{3}|core\.quotepath|quoted.*filename",
        "encoding", "git", ">=2.0", "any", "true", 0.95, 0.95,
        "Git displays Korean/CJK filenames as escaped octal sequences instead of readable characters.",
        [de("Change terminal font to one supporting Korean", "Font is irrelevant; git is escaping the bytes before the terminal renders them",
            0.95,
            sources=["https://git-scm.com/docs/git-config#Documentation/git-config.txt-corequotePath"]),
         de("Set LANG only in .bashrc", "LANG affects terminal encoding but git has its own quotepath setting that overrides display",
            0.70,
            sources=["https://git-scm.com/docs/git-config#Documentation/git-config.txt-corequotePath"])],
        [wa("Set git config --global core.quotepath false", 0.98,
            "git config --global core.quotepath false  # disables octal escaping of non-ASCII filenames",
            sources=["https://git-scm.com/docs/git-config#Documentation/git-config.txt-corequotePath"]),
         wa("Additionally set i18n.logOutputEncoding and i18n.commitEncoding to utf-8", 0.85,
    ...
    canons.append(canon(
        ...
        [de("Wrap in try-catch and skip the file", "Silently skipping files with Korean names hides real data; does not fix the encoding issue",
            0.90,
            sources=["https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/nio/charset/MalformedInputException.html"]),
         de("Convert filename to ASCII with Normalizer", "Normalizer handles decomposition, not transliteration; Korean chars have no ASCII equivalent",
            0.85,
            sources=["https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/text/Normalizer.html"])],
        [wa("Explicitly specify StandardCharsets.UTF_8 in all file read/write operations", 0.95,
         ...
         wa("Set -Dfile.encoding=UTF-8 JVM argument and use InputStreamReader with explicit charset", 0.88,
    ...
    canons.append(canon(
        "pip", "pip-install-nonascii-venv-path", "pip23-linux",
        "pip install fails with UnicodeDecodeError in virtualenv with Korean path",
        r"UnicodeDecodeError.*pip|pip.*install.*[가-힣]|venv.*encode.*error",
        "path_encoding", "pip", ">=23", "linux", "true", 0.85, 0.88,
        "pip install fails when virtualenv is created inside a directory with Korean characters and locale is not UTF-8.",
        [de("Recreate the virtualenv with --clear flag", "Recreating in the same Korean path will reproduce the same encoding error",
            0.90,
            sources=["https://pip.pypa.io/en/stable/cli/pip_install/"]),
         de("Use pip install --no-cache-dir", "Cache is not the issue; the path encoding during extraction is the problem",
            0.85,
            sources=["https://pip.pypa.io/en/stable/cli/pip_install/"])],
        [wa("Create virtualenv in an ASCII path and symlink or set VIRTUAL_ENV", 0.90,
         ...
         wa("Set PYTHONUTF8=1 and ensure system locale is UTF-8 before creating venv", 0.92,
    ...
    canons.append(canon(
        ...
        [de("Use os.system() instead of subprocess", "os.system() is less secure (shell injection) and has the same quoting issues",
            0.90,
            sources=["https://docs.python.org/3/library/subprocess.html"]),
         de("Add backslash escapes to the path string", "Backslash escaping is shell-level; subprocess with shell=False does not interpret shell escapes",
            0.85,
            sources=["https://docs.python.org/3/library/subprocess.html#subprocess.run"])],
        [wa("Pass command as a list of separate arguments, not a single string", 0.95,
            "subprocess.run(['ls', '/path/with spaces/file.txt'])  # list form, not string",
            sources=["https://docs.python.org/3/library/subprocess.html#subprocess.run"]),
         wa("If shell=True is needed, use shlex.quote() to properly escape paths", 0.88,
    ...
    canons.append(canon(
        ...
        [de("Quote the path in CMakeLists.txt only", "CMake may quote it correctly in CMakeLists but the generated Makefile may not propagate quotes to all shell commands",
            0.70,
            sources=["https://cmake.org/cmake/help/latest/manual/cmake.1.html"]),
         de("Replace spaces with underscores in cmake -S argument", "This changes the actual path; cmake cannot find the source directory at the modified path",
            0.95,
            sources=["https://cmake.org/cmake/help/latest/manual/cmake.1.html"])],
    ...
    canons.append(canon(
        ...
        [de("Escape spaces with backslash in COPY instruction", "Backslash escaping is inconsistent across Docker versions and platforms",
            0.75,
            sources=["https://docs.docker.com/reference/dockerfile/#copy"]),
         de("Use shell glob to match the file with spaces", "COPY does not expand shell globs the same way; glob matching is for ADD with URLs only",
            0.85,
            sources=["https://docs.docker.com/reference/dockerfile/#copy"])],
        [wa("Use JSON array form for COPY with paths containing spaces", 0.95,
            'COPY ["src/my file.txt", "/app/my file.txt"]  # JSON array form handles spaces correctly',
            sources=["https://docs.docker.com/reference/dockerfile/#copy"]),
         wa("Rename files to remove spaces before COPY, or use .dockerignore and a staging step", 0.82,
    ...
    canons.append(canon(
        ...
        [de("Quote JAVA_HOME in gradle.properties", "gradle.properties does not interpret shell quotes; the literal quote characters become part of the path",
            0.90,
            sources=["https://docs.gradle.org/current/userguide/build_environment.html"]),
         de("Set JAVA_HOME with Windows short name (PROGRA~1)", "Short names are not always enabled on all volumes and may not resolve correctly",
            0.65,
            sources=["https://docs.gradle.org/current/userguide/build_environment.html"])],
        [wa("Install JDK to a path without spaces (e.g., /opt/jdk17 or C:\\jdk17)", 0.93,
            "export JAVA_HOME=/opt/jdk-17  # avoid 'Program Files' or any path with spaces",
            sources=["https://docs.gradle.org/current/userguide/build_environment.html"]),
         wa("Use org.gradle.java.home in gradle.properties with forward slashes and no spaces", 0.88,
    ...
    canons.append(canon(
        ...
        [de("Use encodeURIComponent on the path", "File system APIs expect OS paths, not URI-encoded strings; %20 is treated as literal characters",
            0.95,
            sources=["https://nodejs.org/api/path.html"]),
         de("Replace spaces with %20 in package.json scripts", "Shell and Node resolve %20 as literal text, not as space replacement",
            0.95,
            sources=["https://docs.npmjs.com/cli/v10/using-npm/scripts"])],
        [wa("Move project to a path without spaces or create a junction/symlink", 0.92,
            "mklink /J C:\\project \"C:\\My Documents\\project\"  # Windows junction; ln -s on Linux",
            sources=["https://nodejs.org/api/path.html"]),
         wa("Wrap all path references in double quotes in npm scripts and shell commands", 0.88,
    ...
    canons.append(canon(
        ...
        [de("Escape spaces with backslash in YAML", "YAML backslash escaping is inconsistent; most actions parse paths as plain strings",
            0.80,
            sources=["https://docs.github.com/en/actions/using-workflows/workflow-syntax-for-github-actions"]),
         de("Use shell globbing to match files with spaces", "Glob patterns in actions/upload-artifact do not handle spaces in directory names",
            0.75,
            sources=["https://github.com/actions/upload-artifact"])],
        [wa("Quote the path and use GitHub Actions expression syntax for dynamic paths", 0.90,
            'path: "build output/artifacts"  # YAML quoted string preserves spaces',
            sources=["https://github.com/actions/upload-artifact"]),
         wa("Rename output directories to avoid spaces in CI workflow", 0.88,
    ...
    canons.append(canon(
        ...
        [de("Use __import__('my-module') for hyphenated names", "Works but produces unidiomatic code; breaks IDE autocompletion and type checking",
            0.60,
            sources=["https://docs.python.org/3/reference/import.html"]),
         de("Add the module to sys.path with importlib", "importlib.import_module('my-module') works but creates maintenance burden and hides dependencies",
            0.55,
            sources=["https://docs.python.org/3/library/importlib.html"])],
        [wa("Rename the package directory: replace hyphens with underscores", 0.98,
         ...
         wa("If third-party package, use importlib.import_module() as last resort", 0.75,
    ...
    canons.append(canon(
        ...
        [de("Use the directory name as-is in package declaration", "Go compiler rejects hyphens in package names; package mypackage in dir my-package causes import confusion",
            0.90,
            sources=["https://go.dev/ref/spec#PackageClause"]),
         de("Use replace directive to map hyphenated path", "Replace directive remaps modules, not individual packages within a module; does not fix the naming issue",
            0.80,
            sources=["https://go.dev/ref/mod#go-mod-file-replace"])],
        [wa("Rename directory to use underscores or no separator, match package declaration name", 0.95,
         ...
         wa("If external module, import by module path and alias the package name", 0.82,
    ...
    canons.append(canon(
        ...
        [de("Change require to use underscore version", "If the actual folder has hyphens, changing to underscores creates a mismatch",
            0.90,
            sources=["https://nodejs.org/api/modules.html"]),
         de("Add 'main' field in sub-folder package.json", "Adding package.json in every sub-folder creates unnecessary complexity for a naming issue",
            0.70,
            sources=["https://nodejs.org/api/packages.html#main"])],
        [wa("Use consistent casing and exact folder name in require/import path", 0.90,
            "import MyComponent from './my-component/index.js'  // explicit exact path",
            sources=["https://nodejs.org/api/esm.html"]),
         wa("Configure module aliases in tsconfig/webpack to abstract the hyphenated path", 0.85,
    ...
    canons.append(canon(
        ...
        [de("Use hyphens in use statement matching Cargo.toml", "Rust identifiers cannot contain hyphens; the compiler parses my-crate as my minus crate",
            0.98,
            sources=["https://doc.rust-lang.org/cargo/reference/specifying-dependencies.html"]),
         de("Rename the Cargo.toml package name to remove hyphens", "This changes the published crate name; breaks existing dependents and crates.io registry entry",
            0.70,
            sources=["https://doc.rust-lang.org/cargo/reference/manifest.html#the-name-field"])],
        [wa("Use underscores in Rust source code: use my_crate (Cargo auto-maps hyphens to underscores)", 0.98,
            "use my_crate::something;  // Cargo.toml has [dependencies] my-crate = \"1.0\" but Rust uses underscores",
            sources=["https://doc.rust-lang.org/cargo/reference/specifying-dependencies.html"]),
         wa("If renaming is needed, use the [lib] name field in Cargo.toml to set a custom lib name", 0.82,
    ...
    canons.append(canon(
        "elasticsearch", "shard-allocation-failed", "es8-linux",
        "ClusterHealthResponse: status=RED, unassigned_shards=5, reason=ALLOCATION_FAILED",
        r"(unassigned_shards|ALLOCATION_FAILED|shard.*not.*allocat|cluster.*health.*RED|no valid shard copy|all copies.*failed)",
        "cluster_error", "elasticsearch", ">=8.0", "linux", "true", 0.78, 0.85,
        "Cluster red or yellow due to unassigned shards. Disk full, node left, or allocation filtering misconfigured.",
        [de("Force-allocate primary shards with accept_data_loss",
            "Force allocation of stale primary shards risks losing acknowledged writes and creating split-brain data", 0.88,
            sources=["https://www.elastic.co/guide/en/elasticsearch/reference/current/cluster-reroute.html"]),
         de("Restart all nodes simultaneously to fix red cluster",
            "Simultaneous restart causes full cluster outage and risks data loss if quorum is lost", 0.82,
            sources=["https://www.elastic.co/guide/en/elasticsearch/reference/current/restart-cluster.html"])],
        [wa("Check allocation explain API for root cause", 0.92,
            "GET _cluster/allocation/explain?pretty  # shows exactly why shard is unassigned"),
         wa("Free disk space below watermark thresholds", 0.88,
            "Default watermarks: low=85%, high=90%, flood=95%. Free disk or adjust cluster.routing.allocation.disk.watermark settings"),
         wa("Reroute with retry_failed to reattempt allocation after fixing root cause", 0.85,
            "POST _cluster/reroute?retry_failed=true  # after resolving disk/node issues")],
    ))

    # Elasticsearch: search phase execution exception
    canons.append(canon(
        "elasticsearch", "search-phase-execution-exception", "es8-linux",
        "SearchPhaseExecutionException: all shards failed: [query_shard_exception] Reason: failed to create query",
        r"(SearchPhaseExecutionException|all shards failed|query_shard_exception|search.*phase.*execution|failed to create query)",
        "query_error", "elasticsearch", ">=8.0", "linux", "true", 0.85, 0.88,
        "Search query failed across all shards. Malformed query DSL, field type mismatch, or corrupt shard data.",
        [de("Retry the same query assuming transient failure",
            "If query syntax is wrong, retrying produces the same error every time. Check the root cause first.", 0.75,
            sources=["https://www.elastic.co/guide/en/elasticsearch/reference/current/query-dsl.html"]),
         de("Delete and reindex to fix corrupt shard data",
            "Reindexing is expensive and causes downtime. Most search failures are query issues, not data corruption.", 0.70,
            sources=["https://www.elastic.co/guide/en/elasticsearch/reference/current/docs-reindex.html"])],
        [wa("Check root cause in the nested 'caused_by' field of the error response", 0.92,
            "The top-level message is generic; look at response.error.root_cause[0].reason for specifics"),
         wa("Validate query against mapping: ensure field names and types match", 0.88,
            "GET /my_index/_mapping  # then verify field types match your query expectations"),
         wa("Use _validate/query API to debug query syntax", 0.85,
            "GET /my_index/_validate/query?explain=true { 'query': { ... } }")],
    ))

    # Elasticsearch: fielddata disabled on text fields
    canons.append(canon(
        "elasticsearch", "illegal-argument-exception-fielddata", "es8-linux",
        ...
        r"(Fielddata is disabled|illegal_argument.*fielddata|Text fields are not optimised|set fielddata=true|fielddata.*aggregat)",
        "mapping_error", "elasticsearch", ">=8.0", "linux", "true", 0.90, 0.90,
        ...
        [de("Enable fielddata=true on the text field",
            "Fielddata on text fields loads all unique terms into JVM heap. Causes massive memory usage and OOM on high-cardinality fields.", 0.90,
            sources=["https://www.elastic.co/guide/en/elasticsearch/reference/current/text.html#fielddata-mapping-param"]),
         de("Convert the field to keyword type on an existing index",
            "You cannot change field type on an existing index. Requires reindex to a new index with updated mapping.", 0.85,
            sources=["https://www.elastic.co/guide/en/elasticsearch/reference/current/indices-put-mapping.html"])],
        [wa("Use the .keyword multi-field for aggregations and sorting", 0.95,
            "GET /my_index/_search { 'aggs': { 'top_messages': { 'terms': { 'field': 'message.keyword' }}}}"),
         wa("Add a keyword sub-field to the mapping and reindex", 0.85,
            "PUT /my_index/_mapping { 'properties': { 'message': { 'type': 'text', 'fields': { 'keyword': { 'type': 'keyword' }}}}}"),
         wa("Use composite aggregation with keyword field for large cardinality", 0.80)],
    ))

    # Elasticsearch: cluster block exception
    canons.append(canon(
        "elasticsearch", "cluster-block-exception", "es8-linux",
        ...
        r"(ClusterBlockException|FORBIDDEN.*cluster read-only|cluster.*block|FORBIDDEN/9|FORBIDDEN/13|FORBIDDEN/8.*metadata)",
        "cluster_error", "elasticsearch", ">=8.0", "linux", "true", 0.88, 0.88,
        "Cluster-level write block active. Triggered by disk watermarks, manual settings, or metadata write block during upgrades.",
        [de("Force delete indices to free space while blocks are active",
            "Read-only-allow-delete blocks permit deletion, but cluster-level blocks may prevent even that. Clear the block first.", 0.72,
            sources=["https://www.elastic.co/guide/en/elasticsearch/reference/current/cluster-update-settings.html"]),
         de("Restart nodes to clear the block",
            "Blocks set via persistent settings survive restarts. The block must be cleared via the cluster settings API.", 0.80,
            sources=["https://www.elastic.co/guide/en/elasticsearch/reference/current/modules-cluster.html"])],
        [wa("Clear the cluster read-only block via settings API", 0.95,
            "PUT _cluster/settings { 'persistent': { 'cluster.blocks.read_only_allow_delete': null, 'cluster.blocks.read_only': null } }"),
         wa("Free disk space to clear auto-applied flood watermark blocks", 0.90,
            "Check: GET _cat/allocation?v  # then delete old indices or add disk"),
         wa("Check if upgrade or snapshot operation set a temporary metadata block", 0.82,
            "GET _cluster/settings?include_defaults=true  # look for cluster.blocks.* settings")],
    ))

    # Elasticsearch: rejected execution exception
    canons.append(canon(
        "elasticsearch", "rejected-execution-exception", "es8-linux",
        ...
        r"(EsRejectedExecutionException|rejected execution|bulk.*reject|thread pool.*queue.*full|TOO_MANY_REQUESTS.*queue)",
        "throughput_error", "elasticsearch", ">=8.0", "linux", "true", 0.82, 0.87,
        "Thread pool queue is full, rejecting new operations. Indexing or search rate exceeds cluster capacity.",
        [de("Increase thread pool queue size to a very large value",
            "Large queues just delay rejection while consuming more heap memory. The real issue is ingestion rate exceeding throughput.", 0.80,
            sources=["https://www.elastic.co/guide/en/elasticsearch/reference/current/modules-threadpool.html"]),
         de("Set refresh_interval=-1 to speed up indexing",
            "Disabling refresh makes recently indexed docs invisible to search and does not address queue capacity", 0.65,
            sources=["https://www.elastic.co/guide/en/elasticsearch/reference/current/tune-for-indexing-speed.html"])],
        [wa("Implement client-side backoff and retry on 429/rejected responses", 0.92,
            "Catch 429 TOO_MANY_REQUESTS and retry with exponential backoff; bulk API returns per-item status"),
         wa("Reduce bulk request size and concurrent bulk threads", 0.88,
            "Start with 5-15 MB bulk size and 1-2 bulk threads per node; increase gradually while monitoring rejections"),
         wa("Scale out by adding data nodes to distribute load", 0.82,
            "More shards across more nodes increases write throughput linearly")],
    ))

    # Elasticsearch: snapshot restore failed
    canons.append(canon(
        "elasticsearch", "snapshot-restore-failed", "es8-linux",
        ...
        r"(SnapshotRestoreException|cannot restore index|snapshot.*failed|restore.*failed|snapshot_restore_exception|index.*already exists.*cluster)",
        "snapshot_error", "elasticsearch", ">=8.0", "linux", "true", 0.85, 0.88,
        "Snapshot restore failed. Index name conflicts, incompatible versions, or repository access issues.",
        [de("Delete the existing index to make room for restore",
            "Deleting the live index before restore is confirmed causes data loss if restore also fails. Rename instead.", 0.82,
            sources=["https://www.elastic.co/guide/en/elasticsearch/reference/current/snapshots-restore-snapshot.html"]),
         de("Restore from a different snapshot repository format",
            "Repositories are format-specific. An S3 snapshot cannot be restored from a local fs repo path and vice versa.", 0.75,
            sources=["https://www.elastic.co/guide/en/elasticsearch/reference/current/snapshots-register-repository.html"])],
        [wa("Restore to a different index name using rename_pattern", 0.92,
            "POST _snapshot/repo/snap/_restore { 'rename_pattern': 'my_index', 'rename_replacement': 'restored_my_index' }"),
         wa("Close the existing index before restoring (if acceptable)", 0.85,
            "POST /my_index/_close  # then restore; the closed index will be replaced"),
         wa("Verify snapshot integrity before restore", 0.80,
    ...
    canons.append(canon(
        "grpc", "permission-denied", "grpc1-linux",
        "grpc._channel._InactiveRpcError: StatusCode.PERMISSION_DENIED: Access denied",
        r"(StatusCode\.PERMISSION_DENIED|PERMISSION_DENIED|Access denied|permission.*denied.*grpc|unauthenticated.*token.*invalid)",
        "auth_error", "grpc", ">=1.50", "linux", "true", 0.85, 0.88,
        "gRPC call rejected due to missing or invalid authentication credentials. Token expired, wrong scopes, or misconfigured interceptor.",
        [de("Disable authentication on server for testing",
            "Disabling auth in production is a security vulnerability. Use proper test tokens or a test environment.", 0.88,
            sources=["https://grpc.io/docs/guides/auth/"]),
         de("Hardcode credentials in client source code",
            "Hardcoded credentials get committed to version control and cannot be rotated. Use environment variables or credential files.", 0.82,
            sources=["https://grpc.io/docs/guides/auth/"])],
        [wa("Attach auth metadata via client interceptor or call credentials", 0.92,
            "creds = grpc.access_token_call_credentials(token); channel = grpc.secure_channel(addr, grpc.composite_channel_credentials(grpc.ssl_channel_credentials(), creds))"),
         wa("Verify token validity and expiration before sending", 0.88,
            "Check token expiry time; refresh OAuth2/JWT tokens before they expire"),
         wa("Check server-side auth interceptor configuration and required scopes", 0.82,
            "Ensure the server interceptor expects the same token format and scopes that the client is sending")],
    ))

    # gRPC: resource exhausted (rate limiting)
    canons.append(canon(
        "grpc", "resource-exhausted", "grpc1-linux",
        "grpc._channel._InactiveRpcError: StatusCode.RESOURCE_EXHAUSTED: Quota exceeded; rate limit reached",
        r"(StatusCode\.RESOURCE_EXHAUSTED|RESOURCE_EXHAUSTED.*quota|rate limit.*exceeded|resource.*exhausted|too many requests.*grpc)",
        "throttle_error", "grpc", ">=1.50", "linux", "true", 0.82, 0.85,
        "gRPC call rejected due to rate limiting or resource quota exceeded. Server-side concurrency limit or external quota.",
        [de("Retry immediately without backoff",
            "Immediate retries during rate limiting flood the server and extend the backoff window. Use exponential backoff.", 0.90,
            sources=["https://grpc.io/docs/guides/retry/"]),
         de("Open multiple channels to bypass rate limits",
            "Server-side rate limits typically apply per-user or per-API-key, not per-connection. Multiple channels hit the same limit.", 0.78,
            sources=["https://grpc.io/docs/guides/performance/"])],
        [wa("Implement exponential backoff with jitter on RESOURCE_EXHAUSTED", 0.92,
            "Retry with base_delay * 2^attempt + random_jitter; respect Retry-After header if present in trailing metadata"),
         wa("Use client-side rate limiting to stay below quota", 0.88,
            "Implement a token bucket or leaky bucket limiter on the client side to smooth out request bursts"),
         wa("Request quota increase from the service provider if limits are too low", 0.80)],
    ))

    # gRPC: internal error
    canons.append(canon(
        "grpc", "internal-error", "grpc1-linux",
        "grpc._channel._InactiveRpcError: StatusCode.INTERNAL: Internal Server Error",
        r"(StatusCode\.INTERNAL|INTERNAL.*[Ss]erver [Ee]rror|grpc.*internal.*error|INTERNAL.*exception|grpc.*status.*13)",
        "server_error", "grpc", ">=1.50", "linux", "true", 0.75, 0.82,
        "Server-side unhandled exception or internal failure. The server crashed, panicked, or returned a generic error without details.",
        [de("Catch all exceptions in client and ignore INTERNAL status",
            "Silencing INTERNAL errors hides server-side bugs that need fixing. Log and alert on these errors.", 0.82,
            sources=["https://grpc.io/docs/guides/error/"]),
         de("Increase client timeout assuming server is slow",
            "INTERNAL status means the server failed, not that it is slow. Longer timeout will not fix a server-side exception.", 0.72,
            sources=["https://grpc.io/docs/guides/deadlines/"])],
        [wa("Check server-side logs for the actual exception and stack trace", 0.92,
            "The gRPC INTERNAL status is generic. The real error is in server logs, not the client-side message."),
         wa("Add proper error handling in server RPC handlers", 0.88,
            "Catch exceptions in handler; return grpc.StatusCode.INTERNAL with a descriptive message via context.abort()"),
         wa("Enable gRPC server-side debug logging", 0.82,
            "GRPC_VERBOSITY=DEBUG GRPC_TRACE=all python server.py  # or set channelz for production observability")],
    ))

    # gRPC: cancelled context
    canons.append(canon(
        "grpc", "cancelled-context", "grpc1-linux",
        "grpc._channel._InactiveRpcError: StatusCode.CANCELLED: Context cancelled",
        r"(StatusCode\.CANCELLED|CANCELLED.*[Cc]ontext|context.*cancel|grpc.*cancelled|client.*cancel.*rpc)",
        "cancellation_error", "grpc", ">=1.50", "linux", "true", 0.85, 0.88,
        "RPC was cancelled by the client. Parent context cancelled, user navigated away, or explicit cancellation.",
        [de("Retry cancelled RPCs unconditionally on the client side",
            "If the client explicitly cancelled (e.g., user action), retrying wastes resources and may cause duplicate side effects.", 0.78,
            sources=["https://grpc.io/docs/guides/cancellation/"]),
         de("Ignore cancellation errors in the server handler",
            "Ignoring cancellation means the server continues expensive work that nobody will read. Always check context.is_active().", 0.80,
            sources=["https://grpc.io/docs/guides/cancellation/"])],
        [wa("Handle cancellation gracefully on the server side", 0.92,
            "Check context.is_active() in long-running handlers; abort work early when cancelled to free resources"),
         wa("Distinguish intentional vs. unintentional cancellation on client", 0.85,
            "Track whether cancellation came from user action (no retry) or parent context (possible retry)"),
         wa("Use context.add_callback() to clean up resources on cancellation", 0.82,
            "context.add_callback(cleanup_fn)  # Python server; runs when RPC is cancelled")],
    ))

    # gRPC: failed precondition
    canons.append(canon(
        "grpc", "failed-precondition", "grpc1-linux",
        "grpc._channel._InactiveRpcError: StatusCode.FAILED_PRECONDITION: Operation rejected, system is not in required state",
        r"(StatusCode\.FAILED_PRECONDITION|FAILED_PRECONDITION|precondition.*failed|system.*not.*required state|operation.*rejected.*state)",
        "state_error", "grpc", ">=1.50", "linux", "true", 0.82, 0.87,
        "Server rejected the operation because a required precondition was not met. Wrong system state, missing setup step, or ordering violation.",
        [de("Retry the same RPC without changing state",
            "FAILED_PRECONDITION means state must change before the operation can succeed. Blind retry hits the same error.", 0.88,
            sources=["https://grpc.io/docs/guides/error/"]),
         de("Ignore the error and continue with next operation",
            "Operations often have ordering dependencies. Skipping a failed precondition may cause cascading failures.", 0.75,
            sources=["https://grpc.io/docs/guides/error/"])],
        [wa("Read the error details to determine which precondition failed", 0.92,
            "Check status.details() or trailing metadata for specific precondition information from the server"),
         wa("Ensure required setup or ordering steps are completed first", 0.88,
            "Common causes: resource not initialized, wrong lifecycle state, missing prior API call"),
         wa("Implement a state machine on the client to track valid operation ordering", 0.80)],
    ))

    # gRPC: not found service
    canons.append(canon(
        "grpc", "not-found-service", "grpc1-linux",
        "grpc._channel._InactiveRpcError: StatusCode.NOT_FOUND: Service not found",
        r"(StatusCode\.NOT_FOUND|NOT_FOUND.*[Ss]ervice|service.*not.*found|method.*not.*found.*grpc|unknown service)",
        "routing_error", "grpc", ">=1.50", "linux", "true", 0.88, 0.90,
        ...
        [de("Use a different gRPC status code to mask the error",
            "Masking NOT_FOUND with a different code hides the real routing issue and makes debugging harder.", 0.70,
            sources=["https://grpc.io/docs/guides/error/"]),
         de("Add a catch-all handler for unknown services",
            "A catch-all handler bypasses proper service registration and type safety of gRPC. Fix the routing instead.", 0.72,
            sources=["https://grpc.io/docs/guides/"])],
        [wa("Verify the service is registered on the server and listening", 0.95,
    ...
    canons.append(canon(
        "huggingface", "model-not-found", "hf-transformers-linux",
        ...
        r"(is not a local folder|not a valid model identifier|RepositoryNotFoundError|404.*model.*not found|repository.*not.*found.*huggingface)",
        "loading_error", "transformers", ">=4.30", "linux", "true", 0.90, 0.92,
        "Model identifier not found on Hugging Face Hub. Typo in model name, private repo, or model was deleted/renamed.",
        [de("Search for the model on external mirror sites",
            "External mirrors may host outdated, tampered, or license-violating copies. Always use the official Hub.", 0.82,
            sources=["https://huggingface.co/docs/hub/models"]),
         de("Assume the Hub is down and retry later",
            "If the model ID is wrong, retrying will never work. Verify the model name first.", 0.70,
            sources=["https://huggingface.co/docs/huggingface_hub/guides/download"])],
        [wa("Verify exact model ID on huggingface.co/models", 0.95,
            "Model IDs are case-sensitive and include the org: 'meta-llama/Llama-2-7b-hf' not 'llama-2-7b'"),
         wa("Authenticate for private or gated models", 0.88,
            "huggingface-cli login  # then: AutoModel.from_pretrained('model_id', token=True)"),
         wa("Check if model was renamed or deprecated and find the new ID", 0.82,
            "Search huggingface.co/models for the model family; check the model card for migration notes")],
        python=">=3.9",
    ))

    # Hugging Face: pipeline device mismatch
    canons.append(canon(
        "huggingface", "pipeline-device-mismatch", "hf-transformers-linux",
        "RuntimeError: Expected all tensors to be on the same device, but found at least two devices, cuda:0 and cpu!",
        r"(Expected all tensors.*same device|found.*two devices.*cuda.*cpu|device.*mismatch|RuntimeError.*cuda.*cpu|Input.*device.*model.*device)",
        "device_error", "transformers", ">=4.30", "linux", "true", 0.88, 0.90,
        "Model and input tensors are on different devices. Pipeline loaded on CPU while inputs on GPU, or mixed device_map.",
        [de("Move individual tensors with .to() after each operation",
            "Manually moving tensors is error-prone and slow. Use device_map='auto' or ensure consistent device assignment.", 0.75,
            sources=["https://huggingface.co/docs/transformers/main/en/main_classes/pipelines"]),
         de("Set CUDA_VISIBLE_DEVICES to empty to force CPU",
            "Forcing CPU is a workaround, not a fix. The model will run orders of magnitude slower.", 0.70,
            sources=["https://huggingface.co/docs/accelerate/usage_guides/big_modeling"])],
        [wa("Use device_map='auto' in from_pretrained to handle placement automatically", 0.92,
            "model = AutoModelForCausalLM.from_pretrained('model', device_map='auto')"),
         wa("Set device explicitly in pipeline constructor", 0.90,
            "pipe = pipeline('text-generation', model='model', device=0)  # or device='cuda:0'"),
         wa("Ensure input tensors match model device before forward pass", 0.85,
            "inputs = tokenizer(text, return_tensors='pt').to(model.device)  # use model.device for consistency")],
        gpu="any", python=">=3.9",
    ))

    # Hugging Face: config validation error
    canons.append(canon(
        "huggingface", "config-validation-error", "hf-transformers-linux",
        "ValueError: Unrecognized configuration class or invalid config parameter for model type",
        r"(Unrecognized configuration|invalid config|not a valid model type|config.*validation|PretrainedConfig.*error|model_type.*not found)",
        "configuration_error", "transformers", ">=4.30", "linux", "true", 0.85, 0.88,
        "Model configuration is invalid or incompatible. Custom model type not registered, config.json corrupt, or version mismatch.",
        [de("Manually edit config.json to fix unrecognized fields",
            "Editing config.json without understanding the model architecture can cause silent model corruption or wrong outputs.", 0.78,
            sources=["https://huggingface.co/docs/transformers/main_classes/configuration"]),
         de("Downgrade transformers to an older version",
            "Older versions may lack support for newer model architectures. Pin to the version the model was released for.", 0.68,
            sources=["https://huggingface.co/docs/transformers/installation"])],
        [wa("Upgrade transformers to latest version for new model support", 0.90,
            "pip install --upgrade transformers  # new model types require recent library versions"),
         wa("Use trust_remote_code=True for custom model architectures", 0.88,
            "model = AutoModel.from_pretrained('model', trust_remote_code=True)  # enables custom modeling code"),
         wa("Verify config.json matches the expected model_type", 0.82,
            "Check model card for required transformers version; compare config.json with a known-good config")],
        python=">=3.9",
    ))

    # Hugging Face: tokenizer max length exceeded
    canons.append(canon(
        "huggingface", "tokenizer-max-length-exceeded", "hf-transformers-linux",
        ...
        r"(Token indices.*longer than.*maximum|sequence length.*longer|exceed.*max.*length|position.*exceed|indexing errors.*sequence length)",
        "input_error", "transformers", ">=4.30", "linux", "true", 0.90, 0.90,
        "Input text tokenizes to more tokens than the model's maximum sequence length. Causes indexing errors or silent truncation.",
        [de("Increase model_max_length on the tokenizer object",
            "The model's positional embeddings are fixed at training time. Increasing tokenizer max length does not change what the model can process.", 0.88,
            sources=["https://huggingface.co/docs/transformers/main_classes/tokenizer"]),
         de("Ignore the warning assuming the model handles it internally",
            "The model will produce indexing errors or garbage outputs for tokens beyond its position embedding range.", 0.82,
            sources=["https://huggingface.co/docs/transformers/main_classes/tokenizer"])],
        [wa("Enable truncation in the tokenizer call", 0.95,
            "tokens = tokenizer(text, truncation=True, max_length=512, return_tensors='pt')"),
         wa("Split long texts into overlapping chunks and process separately", 0.88,
            "Use a sliding window: chunk text with stride < max_length, process each chunk, aggregate results"),
         wa("Use a model variant with longer context support", 0.82,
            "E.g., use 'model-name-16k' or 'model-name-32k' variants, or models with ALiBi/RoPE for longer contexts")],
        python=">=3.9",
    ))

    # Hugging Face: safetensors load error
    canons.append(canon(
        "huggingface", "safetensors-load-error", "hf-transformers-linux",
        "OSError: Error no file named model.safetensors found in directory or unable to load safetensors file",
        r"(no file named.*safetensors|unable to load safetensors|safetensors.*error|SafetensorError|safetensors.*corrupt|invalid header.*safetensors)",
        "loading_error", "transformers", ">=4.30", "linux", "true", 0.85, 0.88,
        "Failed to load model weights in safetensors format. File missing, corrupted download, or format incompatibility.",
        [de("Convert pickle checkpoint to safetensors manually",
            "Manual conversion can introduce errors with custom architectures. Use the official conversion tool or download the correct format.", 0.72,
            sources=["https://huggingface.co/docs/safetensors/"]),
         de("Ignore safetensors and always use use_safetensors=False",
            "Pickle files have arbitrary code execution risk. Safetensors is the safe default; fix the load issue instead.", 0.78,
            sources=["https://huggingface.co/docs/safetensors/"])],
        [wa("Force re-download by clearing the cache", 0.90,
            "rm -rf ~/.cache/huggingface/hub/models--org--model/  # then re-download"),
         wa("Fall back to PyTorch .bin format if safetensors unavailable", 0.85,
            "model = AutoModel.from_pretrained('model', use_safetensors=False)  # loads pytorch_model.bin instead"),
         wa("Verify file integrity with sha256 checksum", 0.80,
            "sha256sum model.safetensors  # compare with checksum on the model card or Hub API")],
        python=">=3.9",
    ))

    # Hugging Face: attention mask warning
    canons.append(canon(
        "huggingface", "attention-mask-warning", "hf-transformers-linux",
        ...
        r"(strongly recommend.*attention_mask|attention_mask.*padded|no attention mask|Attention mask.*not.*provided|padding.*without.*attention_mask)",
        "input_error", "transformers", ">=4.30", "linux", "true", 0.90, 0.92,
        "Model called without attention mask on padded inputs. Padding tokens will be attended to, producing wrong outputs.",
        [de("Suppress the warning with warnings.filterwarnings",
            "Suppressing the warning does not fix the bug. The model will attend to padding tokens and produce degraded outputs.", 0.90,
            sources=["https://huggingface.co/docs/transformers/troubleshooting"]),
         de("Pad inputs but skip attention mask assuming the model ignores padding",
            "Most transformer models attend to all tokens by default. Without attention mask, padding tokens corrupt the attention computation.", 0.85,
            sources=["https://huggingface.co/docs/transformers/main_classes/tokenizer"])],
        [wa("Always pass attention_mask from tokenizer output", 0.95,
            "inputs = tokenizer(text, padding=True, return_tensors='pt'); outputs = model(**inputs)  # attention_mask included automatically"),
         wa("Use tokenizer with padding=True and return_attention_mask=True", 0.92,
            "tokenizer(texts, padding=True, truncation=True, return_attention_mask=True, return_tensors='pt')"),
         wa("Set pad_token if tokenizer does not have one", 0.85,
    ...
    canons.append(canon(
        "kafka", "leader-not-available", "kafka3-linux",
        "org.apache.kafka.common.errors.LeaderNotAvailableException: Leader not available for partition my_topic-0",
        r"(LeaderNotAvailableException|Leader not available|leader.*not available.*partition|NO_LEADER|leader election in progress)",
        "broker_error", "kafka", ">=3.0", "linux", "true", 0.85, 0.88,
        "Partition leader not available. Typically occurs after broker restart, during leader election, or when a broker is permanently down.",
        [de("Immediately reassign partitions to other brokers",
            "Leader election happens automatically within seconds. Manual reassignment during transient state causes unnecessary data movement.", 0.78),
         de("Restart all brokers to force a clean leader election",
            "Rolling restart loses availability for all partitions, not just the affected ones. Single-broker recovery is sufficient.", 0.82)],
        [wa("Wait for automatic leader election to complete (usually 5-30 seconds)", 0.92,
            "kafka-metadata.sh --snapshot /var/kafka-logs/__cluster_metadata-0/00000000000000000000.log --cluster-id <id>  # KRaft; or check controller logs"),
         wa("Verify the broker is running and has rejoined the cluster", 0.90,
            "kafka-broker-api-versions.sh --bootstrap-server localhost:9092  # check all brokers respond"),
         wa("Check for under-replicated partitions", 0.85,
            "kafka-topics.sh --describe --under-replicated-partitions --bootstrap-server localhost:9092")],
    ))

    canons.append(canon(
        "kafka", "offset-out-of-range", "kafka3-linux",
        "org.apache.kafka.clients.consumer.OffsetOutOfRangeException: Fetch offset 12345 is out of range for partition my_topic-0",
        r"(OffsetOutOfRangeException|offset.*out of range|fetch offset.*out of range|log start offset.*greater than|OFFSET_OUT_OF_RANGE)",
        "consumer_error", "kafka", ">=3.0", "linux", "true", 0.88, 0.87,
        "Consumer's committed offset no longer exists in broker log. Log was truncated by retention policy or compaction.",
        [de("Disable log retention to prevent future offset loss",
            "Unbounded retention fills disk and eventually crashes the broker. Use retention policies appropriate for consumer lag.", 0.90),
         de("Set auto.offset.reset=none to surface the error",
            "With 'none', the consumer throws an exception and stops entirely instead of recovering. Only useful for exactly-once pipelines.", 0.70)],
        [wa("Set auto.offset.reset=earliest or latest based on use case", 0.92,
            "auto.offset.reset=earliest  # reprocess from beginning; or 'latest' to skip to current"),
         wa("Monitor consumer lag to catch falling behind before offsets expire", 0.88,
            "kafka-consumer-groups.sh --describe --group my_group --bootstrap-server localhost:9092  # check LAG column"),
         wa("Increase log retention to exceed maximum expected consumer downtime", 0.85,
            "log.retention.hours=168  # 7 days; must be longer than longest consumer outage")],
    ))

    canons.append(canon(
        "kafka", "not-enough-replicas", "kafka3-linux",
        "org.apache.kafka.common.errors.NotEnoughReplicasException: Messages are rejected since there are fewer in-sync replicas than required",
        r"(NotEnoughReplicasException|fewer in-sync replicas|NOT_ENOUGH_REPLICAS|min\.insync\.replicas|ISR.*shrunk)",
        "replication_error", "kafka", ">=3.0", "linux", "true", 0.82, 0.88,
        "min.insync.replicas not met. Too many brokers are down or lagging behind the leader.",
        [de("Lower min.insync.replicas to 1",
            "min.insync.replicas=1 with acks=all provides no durability guarantee. A single broker failure can lose acknowledged writes.", 0.85),
         de("Switch producer to acks=0 or acks=1 to bypass ISR check",
            "Bypasses the safety net. Messages are acknowledged before replication, risking data loss on broker failure.", 0.88)],
        [wa("Bring the down brokers back online to restore ISR", 0.92,
            "sudo systemctl start kafka  # on each down broker; check: kafka-topics.sh --describe --topic my_topic"),
         wa("Verify ISR membership and wait for replicas to catch up", 0.88,
            "kafka-topics.sh --describe --topic my_topic --bootstrap-server localhost:9092  # check Isr column"),
         wa("Set appropriate min.insync.replicas for your replication factor", 0.82,
            "For replication-factor=3, set min.insync.replicas=2. Allows 1 broker failure while maintaining durability.")],
    ))

    canons.append(canon(
        "kafka", "connection-refused-bootstrap", "kafka3-linux",
        ...
        r"(Connection.*could not be established|Failed to construct kafka|bootstrap.*connection refused|Connection refused.*9092|broker.*not available.*bootstrap)",
        "connection_error", "kafka", ">=3.0", "linux", "true", 0.90, 0.90,
        ...
        [de("Use the ZooKeeper address as bootstrap server",
            "Clients connect to Kafka brokers, not ZooKeeper. ZooKeeper port (2181) is not the broker port (9092).", 0.92),
         de("Only check if broker is running and assume network is fine",
            "advertised.listeners misconfiguration is the #1 cause in Docker/cloud. Broker runs fine but clients cannot reach the advertised address.", 0.75)],
        [wa("Verify broker is running and listening on expected port", 0.92,
            "ss -tlnp | grep 9092; sudo journalctl -u kafka --tail=50"),
         wa("Check advertised.listeners matches how clients reach the broker", 0.90,
            "In server.properties: advertised.listeners=PLAINTEXT://actual-hostname:9092  # must be routable from client"),
         wa("Test connectivity from client machine", 0.85,
            "nc -zv broker-host 9092  # or: kafkacat -b broker-host:9092 -L")],
    ))

    canons.append(canon(
        "kafka", "deserialization-error", "kafka3-linux",
        ...
        r"(SerializationException|Error deserializing|deserialization error|DeserializationException|cannot deserialize.*message|seek past the record)",
        "serialization_error", "kafka", ">=3.0", "linux", "true", 0.80, 0.85,
        ...
        [de("Skip the bad message by seeking past it without logging",
            "Silently losing messages leads to data gaps. Always log or dead-letter failed messages for later analysis.", 0.80),
         de("Change the consumer deserializer to StringDeserializer as a quick fix",
            "Masking the format mismatch pushes the parsing failure downstream. Fix the root cause (schema mismatch) instead.", 0.72)],
        [wa("Implement an ErrorHandlingDeserializer to catch and route bad messages", 0.88,
         ...
         wa("Verify producer and consumer use matching serializer/deserializer", 0.90,
            "Producer: value.serializer=AvroSerializer; Consumer: value.deserializer=AvroDeserializer  # must match"),
         wa("Use Schema Registry to enforce schema compatibility", 0.82,
            "schema.registry.url=http://localhost:8081; value.subject.name.strategy=TopicNameStrategy")],
    ))

    canons.append(canon(
        "kafka", "topic-authorization-failed", "kafka3-linux",
        "org.apache.kafka.common.errors.TopicAuthorizationException: Not authorized to access topics: [my_topic]",
        r"(TopicAuthorizationException|Not authorized to access topic|TOPIC_AUTHORIZATION_FAILED|authorization failed.*topic|ACL.*denied.*topic)",
        "auth_error", "kafka", ">=3.0", "linux", "true", 0.90, 0.90,
        "Kafka ACLs deny the client access to the topic. Missing ACL entry, wrong principal, or authorizer misconfigured.",
        [de("Disable the authorizer to make it work",
            "Disabling authorization removes all access control. Any client can read/write any topic. Security risk in production.", 0.92),
         de("Grant all permissions with a wildcard ACL on all resources",
            "Wildcard ACLs defeat the purpose of access control. Grant minimum required permissions per principal.", 0.80)],
        [wa("Add the specific ACL for the client principal", 0.92,
            "kafka-acls.sh --add --allow-principal User:my-app --operation Read --operation Describe --topic my_topic --bootstrap-server localhost:9092"),
         wa("Verify the client is authenticating with the correct principal", 0.88,
            "Check JAAS config: sasl.jaas.config=...username=\"my-app\"...; kafka-acls.sh --list --topic my_topic"),
         wa("Check authorizer logs for the exact denial reason", 0.82,
            "grep 'Denied' /var/log/kafka/kafka-authorizer.log  # shows principal, operation, resource")],
    ))
    ...
    canons.append(canon(
        "mongodb", "write-conflict", "mongo7-linux",
        ...
        r"(WriteConflict|write conflict|WriteConflictException|conflicted with another operation|TransientTransactionError.*WriteConflict)",
        "transaction_error", "mongodb", ">=7.0", "linux", "true", 0.85, 0.88,
        "Write conflict during multi-document transaction or concurrent update. WiredTiger uses optimistic concurrency; conflicts require retry.",
        [de("Use WriteConcern w:0 to avoid conflicts",
            "WriteConcern controls acknowledgment, not concurrency. w:0 means fire-and-forget but does not prevent conflicts.", 0.82),
         de("Wrap every single operation in a transaction",
            "Transactions increase conflict probability. Use transactions only when atomicity across multiple documents is required.", 0.75)],
        [wa("Implement retry logic with exponential backoff for transient errors", 0.92,
            "session.withTransaction(callback, { maxCommitTimeMS: 5000 })  # driver auto-retries TransientTransactionError"),
         wa("Reduce transaction scope and duration", 0.88,
            "Keep transactions short. Read data before starting, minimize writes inside the transaction, avoid network calls."),
         wa("Use findOneAndUpdate for single-document atomic operations", 0.85,
            "db.orders.findOneAndUpdate({ _id: id, status: 'pending' }, { $set: { status: 'shipped' } })  # atomic, no transaction needed")],
    ))

    canons.append(canon(
        "mongodb", "cursor-not-found", "mongo7-linux",
        "MongoServerError: cursor id 1234567890 not found",
        r"(cursor.*not found|CursorNotFound|cursor id.*not found|getMore.*cursor.*not found|cursor has been killed)",
        "query_error", "mongodb", ">=7.0", "linux", "true", 0.85, 0.87,
        "Server-side cursor expired (default 10 min timeout) or was killed. Common when iterating large result sets slowly.",
        [de("Set noCursorTimeout on all queries",
            "Cursors with no timeout are never cleaned up if the client crashes. Leaks memory and resources on the server.", 0.85),
         de("Increase cursor timeout to hours",
            "Long-lived cursors hold resources and pin old data in WiredTiger cache. Process data faster instead.", 0.72)],
        [wa("Use batched processing with skip/limit or range queries", 0.90,
            "Process in pages: db.collection.find({ _id: { $gt: lastId } }).limit(1000)  # keyset pagination, no cursor timeout risk"),
         wa("Use aggregation with $merge or $out for large data processing", 0.88,
            "db.collection.aggregate([{ $match: ... }, { $merge: 'output_collection' }])  # server-side, no cursor needed"),
         wa("Increase batchSize to reduce round trips if processing is fast", 0.82,
            "db.collection.find().batchSize(5000)  # fewer getMore calls means less time for cursor to expire")],
    ))

    canons.append(canon(
        "mongodb", "document-too-large", "mongo7-linux",
        "MongoServerError: BSONObjectTooLarge: object to insert exceeds maximum BSON document size (16793600 > 16777216)",
        r"(BSONObjectTooLarge|exceeds maximum.*size|document.*too large|object to insert exceeds|16777216|16MB.*limit)",
        "data_error", "mongodb", ">=7.0", "linux", "true", 0.88, 0.90,
        "Document exceeds MongoDB's 16MB BSON size limit. Usually caused by unbounded arrays growing over time.",
        [de("Increase the BSON document size limit",
            "The 16MB limit is a hard server-side constraint compiled into MongoDB. It cannot be changed via configuration.", 0.95),
         de("Compress the document before insertion",
            "BSON size is measured after serialization. Application-level compression does not bypass the BSON size limit check.", 0.82)],
        [wa("Refactor unbounded arrays into separate collection with references", 0.92,
            "Instead of: { comments: [...10000] }, use comments collection: { postId: ObjectId, text: '...' }"),
         wa("Use GridFS for large binary data", 0.90,
            "const bucket = new GridFSBucket(db); bucket.openUploadStream('largefile.bin')  # chunks data across documents"),
         wa("Use the bucket pattern to limit array growth", 0.85,
            "Split into buckets: { postId: 1, bucket: 0, comments: [...200] }, { postId: 1, bucket: 1, comments: [...200] }")],
    ))

    canons.append(canon(
        "mongodb", "index-build-failed", "mongo7-linux",
        "MongoServerError: Index build failed: E11000 duplicate key error collection: mydb.users index: email_1 dup key: { email: null }",
        r"(Index build failed|index.*build.*failed|createIndex.*failed|duplicate key.*index build|Cannot build index.*duplicate)",
        "index_error", "mongodb", ">=7.0", "linux", "true", 0.85, 0.88,
        "Index creation failed. Commonly due to duplicate values (including null) when creating a unique index, or insufficient disk/memory.",
        [de("Force the index build with a hidden flag",
            "There is no force flag. The duplicate data must be resolved before a unique index can be created.", 0.90),
         de("Drop and recreate the collection to fix duplicates",
            "Destroying data is not necessary. Clean up duplicates with an aggregation pipeline and targeted deletes.", 0.85)],
        [wa("Find and remove duplicate values before creating the unique index", 0.92,
            "db.users.aggregate([{ $group: { _id: '$email', count: { $sum: 1 }, ids: { $push: '$_id' } } }, { $match: { count: { $gt: 1 } } }])"),
         wa("Use a partial index to exclude null values from uniqueness", 0.90,
            "db.users.createIndex({ email: 1 }, { unique: true, partialFilterExpression: { email: { $exists: true, $ne: null } } })"),
         wa("Build the index in the background to avoid blocking operations", 0.82,
            "db.users.createIndex({ email: 1 }, { unique: true })  # MongoDB 4.2+ builds in background by default")],
    ))

    canons.append(canon(
        "mongodb", "sharding-chunk-too-large", "mongo7-linux",
        "MongoServerError: chunk too large to move: data size exceeds maximum chunk size",
        r"(chunk too large|chunk.*exceeds.*maximum|moveChunk.*too large|jumbo.*chunk|chunkSize.*exceeded)",
        "sharding_error", "mongodb", ">=7.0", "linux", "true", 0.78, 0.83,
        "Chunk exceeds maximum size (default 128MB) and cannot be migrated. Caused by poor shard key choice leading to jumbo chunks.",
        [de("Increase chunkSize to accommodate the large chunks",
            "Larger chunks reduce balancer effectiveness. The root issue is shard key cardinality, not chunk size.", 0.80),
         de("Manually move the jumbo chunk with force flag",
            "Force-moving jumbo chunks can cause out-of-memory on the target shard during migration.", 0.75)],
        [wa("Split the jumbo chunk manually", 0.85,
            "sh.splitAt('mydb.collection', { shardKeyField: splitPoint })  # find splitPoint with db.collection.find().sort({shardKey:1}).skip(N)"),
         wa("Refine the shard key to increase cardinality (MongoDB 5.0+)", 0.82,
            "sh.refineCollectionShardKey('mydb.collection', { existingKey: 1, newField: 1 })  # adds suffix field"),
         wa("Clear the jumbo flag after manual splitting", 0.78,
            "db.adminCommand({ clearJumboFlag: 'mydb.collection', bounds: [{ key: MinKey }, { key: MaxKey }] })")],
    ))

    canons.append(canon(
        "mongodb", "network-timeout", "mongo7-linux",
        "MongoNetworkTimeoutError: connection timed out; Socket operation timed out after 30000ms",
        r"(MongoNetworkTimeoutError|connection timed out|socket.*timed out|MongoTimeoutError.*network|socketTimeoutMS|ETIMEDOUT.*mongo)",
        "connection_error", "mongodb", ">=7.0", "linux", "true", 0.82, 0.87,
        "Socket or connection timeout during MongoDB operation. Slow query, network issue, or server under heavy load.",
        [de("Set socketTimeoutMS to 0 (infinite)",
            "Infinite timeout means a stuck connection hangs forever, eventually exhausting the connection pool.", 0.88),
         de("Increase connection pool size assuming timeouts mean pool exhaustion",
            "More connections to an overloaded server makes it worse. Fix the slow queries or scale the server.", 0.72)],
        [wa("Identify and optimize slow queries causing timeouts", 0.90,
            "db.currentOp({ 'secs_running': { $gt: 10 } })  # find slow operations; db.setProfilingLevel(1, { slowms: 100 })"),
         wa("Set appropriate timeout values for different operation types", 0.88,
            "socketTimeoutMS=60000; connectTimeoutMS=10000; serverSelectionTimeoutMS=15000  # in connection string"),
         wa("Check network path between application and MongoDB", 0.82,
            "ping mongo-host; traceroute mongo-host; check security group rules and VPC peering if in cloud")],
    ))
    ...
    canons.append(canon(
        "nginx", "ssl-certificate-error", "nginx1.24-linux",
        ...
        r"(SSL_CTX_use_certificate|SSL.*certificate.*failed|SSL.*error.*PEM|cannot load certificate|ssl_certificate.*failed|PEM_read_bio)",
        "ssl_error", "nginx", ">=1.24", "linux", "true", 0.90, 0.90,
        "SSL certificate cannot be loaded. File not found, wrong permissions, invalid PEM format, or incomplete certificate chain.",
        [de("Generate a self-signed certificate for production",
            "Self-signed certificates trigger browser warnings and are rejected by most clients. Use Let's Encrypt for free valid certificates.", 0.82),
         de("Set ssl_verify_client off assuming it fixes the error",
            "ssl_verify_client controls client certificate verification, not server certificate loading. Different directive entirely.", 0.88)],
        [wa("Check file permissions and ownership on certificate files", 0.92,
            "ls -la /etc/nginx/ssl/; chown root:root cert.pem key.pem; chmod 644 cert.pem; chmod 600 key.pem"),
         wa("Verify the certificate chain is complete and in correct order", 0.90,
         ...
         wa("Validate PEM format and key match", 0.85,
            "openssl x509 -noout -modulus -in cert.pem | md5sum; openssl rsa -noout -modulus -in key.pem | md5sum  # must match")],
    ))

    canons.append(canon(
        "nginx", "worker-connections-exceeded", "nginx1.24-linux",
        "nginx: [alert] 1024 worker_connections are not enough",
        r"(worker_connections are not enough|worker_connections.*not enough|accept\(\) failed.*Resource temporarily unavailable|too many connections.*worker)",
        "resource_error", "nginx", ">=1.24", "linux", "true", 0.90, 0.90,
        "nginx worker process ran out of connection slots. Default 1024 is too low for high-traffic sites.",
        [de("Set worker_connections to extremely high value like 1000000",
            "Each connection uses memory (~1-2KB). Unreachable high values waste kernel resources. Set based on actual peak + headroom.", 0.72),
         de("Only increase worker_connections without checking ulimit",
            "OS file descriptor limit (ulimit -n) must be >= worker_connections. nginx cannot open more FDs than the OS allows.", 0.85)],
        [wa("Increase worker_connections to match expected concurrency", 0.92,
            "events { worker_connections 4096; }  # in nginx.conf; rule of thumb: 2-4x expected concurrent connections"),
         wa("Increase OS file descriptor limits", 0.90,
         ...
         wa("Enable multi_accept to handle multiple connections per event", 0.82,
            "events { multi_accept on; use epoll; }  # accept all pending connections at once instead of one by one")],
    ))

    canons.append(canon(
        "nginx", "open-too-many-files", "nginx1.24-linux",
        "nginx: [alert] socket() failed (24: Too many open files)",
        r"(Too many open files|EMFILE|socket\(\) failed.*24|open\(\) failed.*24|worker_rlimit_nofile|ulimit.*files.*nginx)",
        "resource_error", "nginx", ">=1.24", "linux", "true", 0.92, 0.90,
        "nginx process hit the OS file descriptor limit. Each connection uses at least 2 FDs (client + upstream). Proxy doubles this.",
        [de("Restart nginx to temporarily fix the issue",
            "Restarting reclaims FDs but the limit is hit again under the same load. Must increase the limit.", 0.85),
         de("Only set ulimit in the shell before starting nginx",
            "Shell ulimit does not persist across service restarts. Must configure in systemd unit or nginx.conf.", 0.80)],
        [wa("Set worker_rlimit_nofile in nginx.conf", 0.92,
            "worker_rlimit_nofile 65536;  # in main context of nginx.conf; sets RLIMIT_NOFILE for worker processes"),
         wa("Increase limits in systemd service unit", 0.90,
         ...
         wa("Set system-wide limits in /etc/security/limits.conf", 0.85,
            "* soft nofile 65536\n* hard nofile 65536\n# Also check: sysctl fs.file-max")],
    ))

    canons.append(canon(
        "nginx", "invalid-pid-number", "nginx1.24-linux",
        "nginx: [error] invalid PID number \"\" in \"/run/nginx.pid\"",
        r"(invalid PID number|nginx\.pid.*invalid|PID file.*not found|stale PID file|open\(\).*nginx\.pid.*failed|no such process.*nginx)",
        "startup_error", "nginx", ">=1.24", "linux", "true", 0.92, 0.92,
        "PID file is empty, stale, or missing. Usually after unclean shutdown, manual file deletion, or containerized nginx.",
        [de("Manually write a PID number into the PID file",
            "Writing an arbitrary PID causes nginx -s reload/stop to signal the wrong process. Let nginx manage its own PID file.", 0.90),
         de("Delete the PID file and hope nginx recreates it",
            "nginx only writes the PID file on startup. Deleting it while running makes nginx -s commands fail until next restart.", 0.72)],
        [wa("Kill any existing nginx processes and start fresh", 0.92,
            "sudo pkill nginx; sleep 1; sudo nginx  # or: sudo nginx -s stop 2>/dev/null; sudo nginx"),
         wa("Use systemctl which manages PID tracking automatically", 0.90,
            "sudo systemctl restart nginx  # systemd tracks the PID independently of nginx.pid"),
         wa("Verify the PID file path matches nginx.conf", 0.82,
            "grep pid /etc/nginx/nginx.conf  # default: /run/nginx.pid; must match systemd ExecStart expectations")],
    ))

    canons.append(canon(
        "nginx", "rewrite-loop-detected", "nginx1.24-linux",
        "nginx: rewrite or internal redirection cycle while processing \"/index.html\"",
        r"(rewrite or internal redirection cycle|rewrite.*cycle|too many redirects|ERR_TOO_MANY_REDIRECTS.*nginx|redirect loop|internal redirect.*loop)",
        "config_error", "nginx", ">=1.24", "linux", "true", 0.88, 0.90,
        "Infinite redirect loop in nginx rewrite rules or try_files. Request bounces between locations or rewrites indefinitely.",
        [de("Add more rewrite rules to try to break the loop",
            "Adding rewrites to fix rewrites increases complexity. Simplify the existing rules instead of layering more.", 0.82),
         de("Use proxy_pass to localhost to escape the rewrite loop",
            "Proxying to self creates a new request that hits the same rewrite rules, causing the same loop.", 0.90)],
        [wa("Add a break or last flag to stop rewrite processing", 0.92,
            "rewrite ^/old(.*)$ /new$1 last;  # 'last' restarts location matching; 'break' stops all rewriting"),
         wa("Use return instead of rewrite for simple redirects", 0.90,
            "return 301 https://$host$request_uri;  # return is simpler and cannot loop within the same server block"),
         wa("Use 'if' with specific conditions to prevent recursive matches", 0.82,
            "if ($request_uri !~ ^/new) { rewrite ^/(.*)$ /new/$1 last; }  # guard against re-matching")],
    ))

    canons.append(canon(
        "nginx", "proxy-read-timeout", "nginx1.24-linux",
        ...
        r"(upstream timed out.*reading response|504 Gateway Time-?out|proxy_read_timeout|Operation timed out.*upstream|upstream.*110.*timed out)",
        "proxy_error", "nginx", ">=1.24", "linux", "true", 0.82, 0.87,
        ...
        [de("Set proxy_read_timeout to 3600 or higher globally",
            "Extremely long timeouts mask backend failures and tie up nginx worker connections for the duration. Set per-location for specific slow endpoints.", 0.75),
         de("Add keepalive connections assuming the timeout is a connection issue",
            "proxy_read_timeout is about response time, not connection establishment. Keepalive helps with proxy_connect_timeout, not read timeout.", 0.80)],
        [wa("Increase proxy_read_timeout for specific slow endpoints only", 0.88,
            "location /api/reports { proxy_read_timeout 300s; proxy_pass http://backend; }  # only slow endpoints get longer timeout"),
         wa("Fix the slow backend response (optimize query, add caching)", 0.92,
            "Profile backend: slow query log, APM tracing. Add caching for expensive operations. 504 means the backend is too slow."),
         wa("Implement request buffering and async processing for long operations", 0.82,
    ...
    # =====================================================================
    canons.append(canon(
        "tensorflow", "failed-to-get-convolution-algorithm", "tf2.15-linux",
        ...
        r"(Failed to get convolution algorithm|cuDNN failed to initialize|CUDNN_STATUS_INTERNAL_ERROR|Could not create cudnn handle|CUDNN_STATUS_NOT_INITIALIZED)",
        "gpu_error", "tensorflow", ">=2.10", "linux", "true", 0.82, 0.88,
        "cuDNN cannot select a convolution algorithm. Usually a cuDNN/CUDA version mismatch or GPU memory already exhausted.",
        [de("Downgrade cuDNN to an older version blindly",
            "cuDNN must match CUDA and TF versions exactly. Arbitrary downgrades cause ABI incompatibilities.", 0.80),
         de("Set TF_CUDNN_USE_AUTOTUNE=0 to skip algorithm selection",
            "Disabling autotuning avoids the error but makes convolutions extremely slow (10-50x slower).", 0.75)],
        [wa("Enable GPU memory growth to prevent cuDNN OOM during init", 0.90,
            "tf.config.experimental.set_memory_growth(tf.config.list_physical_devices('GPU')[0], True)"),
         wa("Install matching cuDNN version for your CUDA/TF combination", 0.88,
            "TF 2.15 requires CUDA 12.2 + cuDNN 8.9. Check tensorflow.org/install/source#gpu"),
         wa("Use pip install tensorflow[and-cuda] for bundled compatible libraries", 0.85,
            "pip install tensorflow[and-cuda]  # auto-installs matching CUDA/cuDNN")],
        gpu="any", python=">=3.9",
    ))

    canons.append(canon(
        "tensorflow", "invalid-argument-shape-mismatch", "tf2.15-linux",
        "tensorflow.python.framework.errors_impl.InvalidArgumentError: Matrix size-incompatible: In[0]: [32,128], In[1]: [256,10]",
        r"(Matrix size-incompatible|InvalidArgumentError.*Matrix|matmul.*incompatible|Dense.*shape.*expected|Input.*incompatible with.*layer)",
        "shape_error", "tensorflow", ">=2.10", "linux", "true", 0.88, 0.90,
        ...
        [de("Add a Reshape layer to force the shapes to match",
            "Reshape does not transform the data meaningfully; it masks the dimensional mismatch and produces garbage output.", 0.82),
         de("Remove the failing layer and hope the model still works",
            "Removing layers changes the architecture fundamentally. Fix the shape chain instead.", 0.78)],
        [wa("Use model.summary() to trace shapes through all layers", 0.92,
            "model.summary()  # trace input/output shapes layer by layer to find where mismatch occurs"),
         wa("Set correct input_shape on the first layer matching your data", 0.88,
            "model.add(Dense(128, input_shape=(feature_dim,)))  # feature_dim must match X_train.shape[1]"),
         wa("Use GlobalAveragePooling2D instead of Flatten for transfer learning", 0.85,
            "base_model.output -> GlobalAveragePooling2D() -> Dense(num_classes)  # avoids shape dependency on input size")],
        python=">=3.9",
    ))

    canons.append(canon(
        "tensorflow", "module-not-found-keras", "tf2.15-linux",
        "ModuleNotFoundError: No module named 'keras' / ImportError: cannot import name 'layers' from 'keras'",
        r"(No module named.*keras|cannot import name.*from.*keras|ModuleNotFoundError.*keras|ImportError.*keras\.layers|keras\.engine.*not found)",
        "import_error", "tensorflow", ">=2.10", "linux", "true", 0.90, 0.90,
        ...
        [de("pip install keras separately alongside tensorflow",
            "TF 2.16+ uses keras 3.x which has breaking API changes. Installing it does not restore tf.keras behavior.", 0.82),
         de("Monkey-patch keras imports with sys.modules",
            "Fragile workaround that breaks with nested imports and keras submodules.", 0.88)],
        [wa("Use tf.keras instead of standalone keras imports", 0.92,
            "from tensorflow.keras.layers import Dense  # NOT from keras.layers import Dense"),
         wa("Pin tf-keras for backward compatibility", 0.88,
            "pip install tf-keras; import os; os.environ['TF_USE_LEGACY_KERAS'] = '1'"),
         wa("Pin TensorFlow < 2.16 if you cannot migrate yet", 0.80,
            "pip install 'tensorflow>=2.15,<2.16'  # last version with bundled keras 2.x")],
        python=">=3.9",
    ))

    canons.append(canon(
        "tensorflow", "aborted-blas-gemm-launch", "tf2.15-linux",
        "InternalError: Blas GEMM launch failed : a]=[32,128], b=[128,64] [Op:MatMul]",
        r"(Blas GEMM launch failed|Blas xGEMM launch failed|InternalError.*GEMM|CUBLAS_STATUS_EXECUTION_FAILED|cublas.*launch)",
        "gpu_error", "tensorflow", ">=2.10", "linux", "true", 0.80, 0.85,
        ...
        [de("Set CUDA_VISIBLE_DEVICES to empty to run on CPU",
            "Avoids GPU entirely. The real fix is resolving the GPU resource conflict.", 0.70),
         de("Reboot the machine to clear GPU state",
            "Only helps if a zombie process holds the GPU. Identify and kill the process instead.", 0.65)],
        [wa("Enable GPU memory growth to avoid pre-allocating all VRAM", 0.88,
            "tf.config.experimental.set_memory_growth(gpu, True)  # prevents TF from claiming all VRAM"),
         wa("Check for other processes using the GPU and kill them", 0.85,
            "nvidia-smi  # check for processes; kill -9 <pid>  # free GPU memory"),
         wa("Ensure cuBLAS version matches CUDA toolkit", 0.82,
            "ldconfig -p | grep cublas  # verify libcublas matches your CUDA version")],
        gpu="any", python=">=3.9",
    ))

    canons.append(canon(
        "tensorflow", "unknown-device-gpu", "tf2.15-linux",
        "RuntimeError: GPU device not found. Could not list physical devices: GPU",
        r"(GPU.*not found|Could not.*physical devices.*GPU|No GPU.*detected|CUDA_ERROR_NO_DEVICE|cannot find.*GPU|device.*GPU:0.*not found)",
        "device_error", "tensorflow", ">=2.10", "linux", "true", 0.85, 0.88,
        ...
        [de("Install CUDA toolkit without the NVIDIA driver",
            "CUDA needs a compatible NVIDIA kernel driver. The toolkit alone is not enough.", 0.85),
         de("Install tensorflow-gpu package (deprecated)",
            "tensorflow-gpu was merged into tensorflow in TF 2.1+. The separate package is abandoned.", 0.78)],
        [wa("Install the NVIDIA driver and verify with nvidia-smi", 0.92,
            "sudo apt install nvidia-driver-535 && nvidia-smi  # must show your GPU"),
         wa("Install tensorflow with CUDA support, not tensorflow-cpu", 0.90,
            "pip install tensorflow[and-cuda]  # NOT pip install tensorflow-cpu"),
         wa("Verify GPU visibility inside Python", 0.85,
            "print(tf.config.list_physical_devices('GPU'))  # should show your GPU; if empty, check driver/CUDA")],
        gpu="any", python=">=3.9",
    ))

    canons.append(canon(
        "tensorflow", "checkpoint-not-found", "tf2.15-linux",
        ...
        r"(Unsuccessful TensorSliceReader|Failed to get matching files.*checkpoint|NotFoundError.*checkpoint|checkpoint.*not found|SavedModel.*not found|No checkpoint found)",
        "io_error", "tensorflow", ">=2.10", "linux", "true", 0.85, 0.88,
        "Checkpoint file missing, path wrong, or only partial checkpoint files present (missing .index or .data).",
        [de("Download only the .ckpt file without .index and .data-00000-of-00001",
            "TF checkpoints are split into 3 files (.index, .data-00000-of-00001, checkpoint). All are required.", 0.90),
         de("Change file extension from .ckpt to .h5 or vice versa",
            "These are completely different formats. .ckpt is TF-native, .h5 is HDF5. They are not interchangeable by renaming.", 0.92)],
        [wa("Verify all checkpoint files exist", 0.92,
            "ls model_dir/  # must have: checkpoint, model.ckpt.index, model.ckpt.data-00000-of-00001"),
         wa("Use tf.train.latest_checkpoint() to find the correct path", 0.88,
            "ckpt_path = tf.train.latest_checkpoint('model_dir/'); model.load_weights(ckpt_path)"),
         wa("Use SavedModel format instead of checkpoints for portability", 0.82,
    ...
    # =====================================================================
    canons.append(canon(
        "unity", "script-compilation-error", "unity2022-any",
        "Assets/Scripts/PlayerController.cs(23,9): error CS1002: ; expected",
        r"(error CS\d{4}|Assets.*\.cs\(\d+.*error|Compilation failed|All compiler errors have to be fixed|scripts have compiler errors)",
        "compilation_error", "unity", ">=2022.3", "cross-platform", "true", 0.92, 0.92,
        ...
        [de("Delete Library folder to force recompilation",
            "Library folder is just a cache. If the script has a syntax error, recompiling produces the same error.", 0.85),
         de("Reimport All assets to fix script errors",
            "Reimport is for asset pipeline issues, not C# syntax errors. Does not recompile scripts.", 0.80)],
        [wa("Read the exact error code and line number in the Console", 0.95,
            "Double-click the error in Console to jump to the exact line in your IDE"),
         wa("Check for IDE/Unity version mismatch with .NET API compatibility", 0.85,
            "Edit > Project Settings > Player > Api Compatibility Level: .NET Standard 2.1 or .NET Framework"),
         wa("Ensure all referenced packages are installed and up to date", 0.82,
            "Window > Package Manager > check for errors or missing packages referenced in your scripts")],
    ))

    canons.append(canon(
        "unity", "asset-import-failed", "unity2022-any",
        "Failed to import asset: 'Assets/Models/character.fbx'. Error: Could not process asset",
        r"(Failed to import|Could not.*process asset|Import failed|asset import.*error|Importer.*Internal.*failed|texture.*import.*failed)",
        "import_error", "unity", ">=2022.3", "cross-platform", "true", 0.82, 0.87,
        "Asset import pipeline failed. Corrupt file, unsupported format, or missing external dependency (e.g., FBX SDK).",
        [de("Rename the file extension to force a different importer",
            "Unity selects importers by content type, not just extension. Renaming causes further confusion.", 0.82),
         de("Delete and re-download the asset repeatedly",
            "If the asset file is corrupt at the source, re-downloading gets the same corrupt file.", 0.70)],
        [wa("Re-export from the source application with compatible settings", 0.90,
            "Blender: Export FBX with 'Apply Transforms' checked; use Binary FBX format, not ASCII"),
         wa("Check the Console for the specific import error details", 0.88,
            "Console > click the error > check Inspector for import settings; try different compression/format"),
         wa("Clear the Library/Artifacts folder to force reimport", 0.82,
            "Close Unity > delete Library/Artifacts/ > reopen project; forces fresh import of all assets")],
    ))

    canons.append(canon(
        "unity", "build-player-failed", "unity2022-any",
        "Build completed with a result of 'Failed'. UnityEditor.BuildPlayerWindow+BuildMethodException",
        r"(Build.*result.*Failed|BuildPlayerWindow|BuildMethodException|Error building Player|Build player.*failed|BuildPipeline.*error)",
        "build_error", "unity", ">=2022.3", "cross-platform", "true", 0.80, 0.85,
        ...
        [de("Switch to a different build target to bypass the error",
            "The error is in your project, not the platform. Switching targets just defers the problem.", 0.75),
         de("Disable all error-producing scripts with #if UNITY_EDITOR",
            "Wrapping everything in UNITY_EDITOR strips the code from builds entirely, causing missing references at runtime.", 0.82)],
        [wa("Check the build log for the first error (not the last)", 0.92,
            "Console > clear > build again > the FIRST red error is the root cause; later errors cascade from it"),
         wa("Ensure all scenes in Build Settings are valid", 0.88,
            "File > Build Settings > verify all scenes exist and are checked; remove missing scenes"),
         wa("Install the target platform SDK/module", 0.85,
            "Unity Hub > Installs > your version > Add Modules > install Android/iOS/WebGL build support")],
    ))

    canons.append(canon(
        "unity", "serialization-layout-mismatch", "unity2022-any",
        "The serialized field layout has changed. Serialization layout for type 'PlayerData' has been changed.",
        r"(serialized field.*layout.*changed|Serialization layout.*changed|serialization.*mismatch|TypeLoadException.*layout|SerializationException.*field)",
        "serialization_error", "unity", ">=2022.3", "cross-platform", "true", 0.85, 0.88,
        "Serialized class layout changed between builds. Saved data (ScriptableObjects, prefabs, scenes) contain stale field definitions.",
        [de("Delete all serialized data files and start fresh",
            "Destroys save data and scene references. Use FormerlySerializedAs to migrate instead.", 0.85),
         de("Revert the class changes to match old serialization",
            "Blocks progress. The correct approach is to migrate the data forward, not lock the code.", 0.72)],
        [wa("Use FormerlySerializedAs attribute when renaming fields", 0.92,
            "[FormerlySerializedAs(\"oldFieldName\")] public int newFieldName;  // preserves serialized data"),
         wa("Implement ISerializationCallbackReceiver for complex migrations", 0.85,
            "OnAfterDeserialize() to convert old data layout to new format on load"),
         wa("Re-serialize affected assets after changing fields", 0.82,
            "Select affected prefabs/ScriptableObjects > right-click > Reimport; or use ForceReserializeAssets API")],
    ))

    canons.append(canon(
        "unity", "package-resolution-error", "unity2022-any",
        "An error occurred while resolving packages: Package com.unity.render-pipelines.universal@14.0.8 has invalid dependencies",
        r"(error.*resolving packages|Package.*invalid dependencies|Package Manager.*resolution|Cannot resolve.*package|Dependency.*could not be resolved|manifest.*conflict)",
        "dependency_error", "unity", ">=2022.3", "cross-platform", "true", 0.85, 0.88,
        "Unity Package Manager cannot resolve package dependencies. Version conflicts, missing registry, or incompatible Unity version.",
        [de("Manually edit Library/PackageCache to fix versions",
            "PackageCache is auto-managed. Manual edits are overwritten immediately on next resolve.", 0.90),
         de("Delete the entire Packages folder",
            "Packages/manifest.json defines your dependencies. Deleting it removes all packages from the project.", 0.88)],
        [wa("Edit Packages/manifest.json to fix version conflicts", 0.92,
            "Open manifest.json > ensure all package versions are compatible with your Unity version; check release notes"),
         wa("Reset package state by deleting Packages/packages-lock.json", 0.88,
            "Delete packages-lock.json > Unity will re-resolve all dependencies from manifest.json on next load"),
         wa("Add or change the scoped registry for third-party packages", 0.82,
            "In manifest.json add scopedRegistries with the correct URL for OpenUPM or custom registries")],
    ))

    canons.append(canon(
        "unity", "il2cpp-build-error", "unity2022-any",
        "IL2CPP error: Unity.IL2CPP.Building.BuilderFailedException: Attempting to constrain an unconstructed type",
        r"(IL2CPP.*error|IL2CPP.*Building|BuilderFailedException|il2cpp\.exe.*failed|IL2CPP.*compilation|UnityLinker.*error|Burst.*IL2CPP)",
        "build_error", "unity", ">=2022.3", "cross-platform", "partial", 0.75, 0.83,
        "IL2CPP AOT compilation failed. Generic type stripping, reflection usage, or unsupported managed code pattern.",
        [de("Switch to Mono backend to avoid IL2CPP issues",
            "Mono is not available on iOS/consoles. IL2CPP is mandatory for most release platforms.", 0.78),
         de("Disable code stripping entirely",
            "Disabling stripping includes everything, making builds 2-5x larger. Use a link.xml instead.", 0.72)],
        [wa("Add a link.xml to preserve types stripped by IL2CPP linker", 0.88,
            "<linker><assembly fullname=\"MyAssembly\" preserve=\"all\"/></linker>  # in Assets/link.xml"),
         wa("Add dummy references for generic types used via reflection", 0.82,
            "private void PreserveTypes() { new List<MyType>(); }  // forces IL2CPP to generate code for this generic"),
         wa("Update IL2CPP / Unity to latest patch for bug fixes", 0.78,
    ...
    # =====================================================================
    canons.append(canon(
        "android", "manifest-merger-failed", "agp8-linux",
        "Execution failed for task ':app:processDebugMainManifest'. Manifest merger failed : Attribute application@appComponentFactory",
        r"(Manifest merger failed|processDebug.*Manifest|Manifest.*merge.*conflict|Attribute.*application@|uses-sdk:minSdkVersion.*cannot be smaller|AndroidManifest.*merge)",
        "build_error", "gradle", ">=8.0", "linux", "true", 0.88, 0.90,
        ...
        [de("Delete the app's AndroidManifest.xml and let it regenerate",
            "The app manifest is the source of truth. Deleting it removes your custom config (permissions, activities, intent filters).", 0.90),
         de("Add tools:replace to every conflicting attribute one by one",
            "Blindly overriding library attributes can break their functionality. Understand what each library needs.", 0.72)],
        [wa("Use tools:replace on specific conflicting attributes", 0.92,
            "<application tools:replace=\"android:appComponentFactory\" ...>  <!-- add xmlns:tools in manifest tag -->"),
         wa("Align minSdkVersion with library requirements", 0.88,
            "Check error for required min SDK; update android { defaultConfig { minSdk = 24 } }  // match library requirement"),
         wa("Use the manifest merger report to find exact conflicts", 0.85,
            "./gradlew processDebugMainManifest --stacktrace; check app/build/outputs/logs/manifest-merger-*-report.txt")],
    ))

    canons.append(canon(
        "android", "dex-method-limit", "agp8-linux",
        "Cannot fit requested classes in a single dex file (# methods: 78291 > 65536)",
        r"(Cannot fit requested classes.*dex|methods.*65536|method.*limit.*exceeded|DexIndexOverflowException|too many.*method references|multidex)",
        "build_error", "gradle", ">=8.0", "linux", "true", 0.95, 0.93,
        "App exceeds the 64K DEX method reference limit. Too many dependencies or minSdk < 21 without multidex.",
        [de("Remove dependencies to get under 65536 methods",
            "Unsustainable for real apps. Multidex is the standard solution and has zero runtime cost on API 21+.", 0.85),
         de("Use ProGuard/R8 minification alone without enabling multidex",
            "R8 helps reduce methods but may not get below 65536. Enable multidex as the reliable fix.", 0.68)],
        [wa("Enable multidex in build.gradle", 0.95,
            "android { defaultConfig { multiDexEnabled true } }  // automatic on minSdk >= 21"),
         wa("For minSdk < 21, add the multidex support library", 0.90,
            "implementation 'androidx.multidex:multidex:2.0.1'; Application class extends MultiDexApplication"),
         wa("Enable R8 minification to reduce method count", 0.85,
            "buildTypes { release { minifyEnabled true; proguardFiles getDefaultProguardFile('proguard-android-optimize.txt') } }")],
    ))

    canons.append(canon(
        "android", "aapt2-resource-linking-failed", "agp8-linux",
        "Android resource linking failed. error: resource style/Theme.AppCompat not found",
        r"(Android resource linking failed|AAPT2.*error|resource.*not found|aapt2.*link.*failed|style.*Theme.*not found|resource.*linking)",
        "build_error", "gradle", ">=8.0", "linux", "true", 0.85, 0.88,
        "AAPT2 cannot link a referenced resource. Missing dependency, wrong theme parent, or resource name typo.",
        [de("Disable AAPT2 (android.enableAapt2=false)",
            "AAPT2 is mandatory since AGP 3.0+. The flag is ignored in modern versions.", 0.92),
         de("Copy missing resource definitions from library source code",
            "Library resources should come from the dependency. Duplicating them causes version conflicts.", 0.78)],
        [wa("Add the missing AppCompat/Material dependency", 0.92,
            "implementation 'com.google.android.material:material:1.11.0'  // provides Material themes and styles"),
         wa("Verify the parent theme exists in your dependencies", 0.88,
            "Check styles.xml parent theme; ensure it matches an available theme from your dependencies"),
         wa("Run AAPT2 with verbose logging to find exact missing resource", 0.82,
            "./gradlew assembleDebug --info 2>&1 | grep 'AAPT'  # shows exact resource and file with the error")],
    ))

    canons.append(canon(
        "android", "ndk-build-failed", "agp8-linux",
        "Build command failed. Error while executing process cmake with arguments: CMake Error at CMakeLists.txt",
        r"(NDK.*build.*failed|CMake Error|cmake.*failed|ndk-build.*error|ninja.*failed|No toolchains found|ABIs.*not supported)",
        "build_error", "gradle", ">=8.0", "linux", "true", 0.78, 0.85,
        "NDK/CMake native build failed. Missing NDK version, wrong CMake config, or native source errors.",
        [de("Install the latest NDK version regardless of project requirements",
            "Projects often require a specific NDK version. Wrong version causes ABI and header incompatibilities.", 0.82),
         de("Delete the .cxx folder and rebuild",
            "Only helps with stale CMake cache. If NDK version or CMakeLists.txt is wrong, same error returns.", 0.65)],
        [wa("Install the exact NDK version specified in build.gradle", 0.92,
            "android { ndkVersion '25.2.9519653' }; install via SDK Manager > SDK Tools > NDK (Side by side)"),
         wa("Verify CMakeLists.txt paths and target architectures", 0.85,
            "Check CMakeLists.txt target_link_libraries and add_library paths; ensure source files exist"),
         wa("Set specific ABI filters to reduce build targets", 0.80,
            "android { defaultConfig { ndk { abiFilters 'arm64-v8a', 'x86_64' } } }  // skip armeabi-v7a if not needed")],
    ))

    canons.append(canon(
        "android", "jetifier-transform-failed", "agp8-linux",
        "Failed to transform artifact 'support-v4.aar' using Jetifier. Reason: Cannot find a mapping for support class",
        r"(Jetifier.*failed|Failed to transform.*Jetifier|Cannot.*mapping.*support class|android\.enableJetifier|support.*AndroidX.*conflict)",
        "build_error", "gradle", ">=8.0", "linux", "true", 0.82, 0.87,
        "Jetifier cannot convert a legacy Android Support Library artifact to AndroidX. Unmapped class or corrupt dependency.",
        [de("Disable Jetifier (android.enableJetifier=false) without migrating",
            "Disabling Jetifier while support-library dependencies remain causes ClassNotFoundException at runtime.", 0.88),
         de("Manually repackage the AAR file with AndroidX namespaces",
            "Fragile and error-prone. Updates to the library will require re-repackaging every time.", 0.85)],
        [wa("Find and replace the old support-library dependency with its AndroidX equivalent", 0.92,
            "Replace 'com.android.support:support-v4:28.0.0' with 'androidx.legacy:legacy-support-v4:1.0.0'"),
         wa("Run the AndroidX migration tool", 0.88,
            "Android Studio: Refactor > Migrate to AndroidX; updates imports and gradle dependencies automatically"),
         wa("Exclude the problematic transitive dependency", 0.80,
            "implementation('com.example:lib:1.0') { exclude group: 'com.android.support', module: 'support-v4' }")],
    ))

    canons.append(canon(
        "android", "signing-config-not-found", "agp8-linux",
        "Could not get unknown property 'release' for SigningConfig container of type org.gradle.api.NamedDomainObjectContainer",
        r"(SigningConfig.*not found|unknown property.*signingConfig|Could not.*SigningConfig|keystore.*not found|signing.*release.*missing|jarsigner.*error)",
        "build_error", "gradle", ">=8.0", "linux", "true", 0.88, 0.90,
        "Release signing configuration missing or keystore file not found. Cannot produce a signed release APK/AAB.",
        [de("Commit the keystore file and passwords to version control",
            "Keystores and passwords are secrets. Committing them exposes your signing identity. Use CI secrets instead.", 0.92),
         de("Generate a new keystore for every release build",
            "Android requires consistent signing across updates. A new keystore means users cannot update the app.", 0.95)],
        [wa("Define signingConfigs block in build.gradle", 0.92,
            "android { signingConfigs { release { storeFile file('keystore.jks'); storePassword STORE_PW; keyAlias 'key0'; keyPassword KEY_PW } } }"),
         wa("Load signing credentials from environment variables or local.properties", 0.90,
            "storePassword System.getenv('STORE_PASSWORD') ?: project.findProperty('storePassword')"),
         wa("Use Gradle's credentials plugin for CI/CD signing", 0.82,
    ...
    # =====================================================================
    canons.append(canon(
        "flutter", "state-not-mounted", "flutter3-linux",
        "setState() called after dispose()",
        r"(setState.*called after dispose|setState\(\) called in.*State.*no longer appears|This widget has been unmounted)",
        "lifecycle_error", "flutter", ">=3.0", "cross-platform", "true", 0.88, 0.90,
        "setState called on a widget that is no longer in the widget tree. Async callback fires after widget is disposed.",
        [de("Wrap setState in try/catch to silence the error",
            "Masks the lifecycle bug; the async operation is still running and wasting resources after dispose", 0.75),
         de("Add mounted check only at the setState call site",
            "Root cause is async operation not being cancelled in dispose(); it keeps running in background", 0.70)],
    ...
    canons.append(canon(
        "flutter", "type-cast-error", "flutter3-linux",
        "type 'Null' is not a subtype of type 'String' in type cast",
        r"(type '.*' is not a subtype of type '.*'( in type cast)?|_CastError|TypeError.*subtype|type cast.*failed)",
        "type_error", "flutter", ">=3.0", "cross-platform", "true", 0.87, 0.89,
        ...
        [de("Cast with 'as' keyword everywhere to force types",
            "'as' throws at runtime when the cast fails; it does not convert types, only asserts them", 0.80),
         de("Disable null safety to avoid type cast errors",
            "Disabling null safety removes all compile-time type checking; makes ALL type issues silent runtime failures", 0.90)],
        [wa("Use safe parsing with null-aware operators and default values", 0.92,
            "final name = json['name'] as String? ?? 'unknown';  // safe cast with fallback"),
         wa("Use fromJson factory constructors with explicit type handling", 0.88,
            "factory User.fromJson(Map<String, dynamic> json) => User(name: json['name']?.toString() ?? '');"),
         wa("Validate API response structure before casting", 0.82,
            "if (json case {'name': String name, 'age': int age}) { ... }  // Dart 3 pattern matching")],
    ))

    canons.append(canon(
        "flutter", "missing-key-properties", "flutter3-linux",
        "Widget rebuild issue: duplicate GlobalKey detected in widget tree",
        r"(Duplicate GlobalKey|Multiple widgets used the same GlobalKey|GlobalKey.*reparented|Keys must be unique|widget.*rebuild.*key)",
        "widget_error", "flutter", ">=3.0", "cross-platform", "true", 0.82, 0.88,
        ...
        [wa("Use ValueKey with a stable unique identifier from the data", 0.92,
         ...
         wa("Use GlobalKey only when you need to access widget state across the tree", 0.80,
            "final _formKey = GlobalKey<FormState>();  // one GlobalKey per widget instance, never reuse")],
    ))

    canons.append(canon(
        "flutter", "platform-channel-error", "flutter3-linux",
        "MissingPluginException(No implementation found for method getBatteryLevel on channel samples.flutter.dev/battery)",
        r"(MissingPluginException.*channel|No implementation found for method.*on channel|PlatformException.*channel|channel-error.*UNAVAILABLE)",
        "platform_error", "flutter", ">=3.0", "cross-platform", "true", 0.83, 0.87,
        "Platform channel method invocation failed. Native side does not have a handler registered for the method channel, or the channel name is misspelled.",
        [de("Add the channel handler in only one platform (e.g., Android but not iOS)",
            "Flutter apps run on multiple platforms; missing handler on any target platform causes runtime crash", 0.75),
         de("Use hot restart to pick up platform channel changes",
            "Hot restart only restarts the Dart VM; native platform code changes require a full rebuild", 0.82)],
        [wa("Verify channel name matches exactly between Dart and native code", 0.92,
            "Dart: MethodChannel('com.example/battery'); Kotlin: MethodChannel(flutterEngine.dartExecutor, \"com.example/battery\")"),
         wa("Do a full stop and rebuild after any native code changes", 0.90,
            "flutter clean && flutter run  // hot restart does NOT reload native platform code"),
         wa("Use Pigeon or federated plugins for type-safe platform channels", 0.82,
            "flutter pub add pigeon  // generates type-safe platform channel code from a schema")],
    ))

    canons.append(canon(
        "flutter", "build-context-deactivated", "flutter3-linux",
        "Looking up a deactivated widget's ancestor is unsafe.",
        r"(Looking up a deactivated widget|deactivated widget's ancestor|This error happens.*BuildContext.*after.*dispose|setState.*deactivated)",
        "lifecycle_error", "flutter", ">=3.0", "cross-platform", "true", 0.85, 0.89,
        ...
        [de("Store BuildContext in a variable and use it later in async code",
            "Context becomes invalid after the widget is deactivated; storing it does not keep it alive", 0.82),
         de("Use a GlobalKey to get context from anywhere",
            "GlobalKey.currentContext can also be null after widget disposal; same lifecycle issue", 0.70)],
        [wa("Check mounted before using context after an await", 0.92,
            "await someAsyncOp(); if (!mounted) return; Navigator.of(context).push(...);"),
         wa("Use a NavigatorKey or router at the app level for navigation from async code", 0.88,
            "final navigatorKey = GlobalKey<NavigatorState>(); // set on MaterialApp, use navigatorKey.currentState!.push(...)"),
         wa("Move async logic to a provider/bloc and react to state changes in the widget", 0.85,
            "Widget listens to state stream and navigates in build/listener, not in async callback")],
    ))

    canons.append(canon(
        "flutter", "dependency-version-solving-failed", "flutter3-linux",
        ...
        r"(version solving failed|Because every version of|incompatible.*constraints|pubspec.*conflict|dependency_overrides|pub.*resolve)",
        "dependency_error", "flutter", ">=3.0", "cross-platform", "true", 0.80, 0.87,
        ...
        [de("Use dependency_overrides to force a specific version",
            "dependency_overrides bypasses version constraints; the overridden package may be incompatible at runtime causing subtle bugs", 0.78),
         de("Pin all packages to exact versions to prevent conflicts",
            "Exact version pins prevent receiving security patches and make future upgrades harder", 0.72)],
        [wa("Run flutter pub outdated and upgrade the conflicting packages", 0.88,
            "flutter pub outdated  # shows which packages have newer versions; flutter pub upgrade --major-versions"),
         wa("Check the dependency tree to find the conflict source", 0.90,
            "flutter pub deps  # shows full dependency tree; look for the package with conflicting constraints"),
         wa("Contact package maintainer or fork the package with updated constraints", 0.75,
    ...
    # =====================================================================
    canons.append(canon(
        "pytorch", "nccl-timeout", "torch2-linux",
        "RuntimeError: NCCL communicator was aborted on rank 2.  Original reason for failure was: watchdog callback timed out",
        r"(NCCL.*abort|NCCL.*timed? ?out|watchdog callback timed out|NCCL communicator.*failure|ProcessGroupNCCL.*timeout|NCCL error.*connection)",
        "distributed_error", "pytorch", ">=2.0", "linux", "true", 0.75, 0.85,
        ...
        [de("Increase NCCL_TIMEOUT to a very large value",
            "Hides the real issue; training hangs silently for minutes before the error surfaces again", 0.72),
         de("Set NCCL_P2P_DISABLE=1 globally",
            "Disabling P2P forces NCCL to use shared memory paths, drastically slowing multi-GPU communication", 0.68)],
        [wa("Check that all ranks reach the collective at the same point", 0.88,
            "Add torch.distributed.barrier() before the collective; print rank and step to find the straggler"),
         wa("Set NCCL_DEBUG=INFO and NCCL_DEBUG_SUBSYS=ALL for detailed diagnostics", 0.85,
            "export NCCL_DEBUG=INFO; look for 'connection refused' or 'timeout' between specific ranks"),
         wa("Verify network interfaces and firewall between nodes", 0.82,
            "export NCCL_SOCKET_IFNAME=eth0; ensure the correct NIC is used; check firewall allows NCCL ports")],
        gpu="any", python=">=3.9",
    ))

    canons.append(canon(
        "pytorch", "autograd-no-grad-fn", "torch2-linux",
        "RuntimeError: element 0 of tensors does not require grad and does not have a grad_fn",
        r"(does not require grad|does not have a grad_fn|element 0 of tensors|can't compute gradient|grad can be implicitly created only for scalar)",
        "autograd_error", "pytorch", ">=2.0", "linux", "true", 0.90, 0.91,
        ...
        [de("Set requires_grad=True on the loss tensor directly",
            "Loss should inherit grad_fn from model parameters via forward pass; setting it manually breaks the chain", 0.80),
         de("Use .data attribute to access tensor values",
            ".data detaches the tensor from autograd; operations on .data do not build a computation graph", 0.85)],
        [wa("Ensure model parameters have requires_grad=True", 0.92,
            "for p in model.parameters(): print(p.requires_grad)  # should all be True; check if optimizer was set up"),
         wa("Verify the forward pass builds a computation graph", 0.90,
            "output = model(input); print(output.grad_fn)  # should not be None; check for .detach() or .item() in forward"),
         wa("Check that torch.no_grad() context is not wrapping the training loop", 0.88,
            "with torch.no_grad(): is for inference only; remove it from training code")],
        python=">=3.9",
    ))

    canons.append(canon(
        "pytorch", "cudnn-rnn-backward-error", "torch2-linux",
        "RuntimeError: cuDNN error: CUDNN_STATUS_EXECUTION_FAILED when calling cudnnRNNBackwardData",
        r"(cuDNN.*RNN.*Backward|CUDNN_STATUS_EXECUTION_FAILED.*RNN|cudnnRNN.*error|cuDNN.*LSTM.*backward|RNN.*backward.*cuDNN)",
        "cuda_error", "pytorch", ">=2.0", "linux", "true", 0.78, 0.85,
        ...
        [de("Disable cuDNN entirely with torch.backends.cudnn.enabled=False",
            "Disabling cuDNN massively slows down RNN operations (10-50x slower); fix the root cause instead", 0.72),
         de("Downgrade to a much older PyTorch version",
            "Older versions have different cuDNN bugs; the real fix is proper input handling", 0.65)],
        [wa("Make hidden states contiguous before passing to RNN", 0.90,
            "hidden = hidden.contiguous()  # or h_0, c_0 = h_0.contiguous(), c_0.contiguous()"),
         wa("Use pack_padded_sequence for variable-length inputs", 0.88,
            "packed = nn.utils.rnn.pack_padded_sequence(input, lengths, batch_first=True, enforce_sorted=False)"),
         wa("Set torch.backends.cudnn.deterministic=True as a diagnostic step", 0.80,
            "If deterministic mode works, the issue is a cuDNN non-deterministic kernel bug; update CUDA/cuDNN")],
        gpu="any", python=">=3.9",
    ))

    canons.append(canon(
        "pytorch", "torchscript-not-supported", "torch2-linux",
        "torch.jit.frontend.UnsupportedNodeError: Set is not supported in TorchScript",
        r"(UnsupportedNodeError|not supported in TorchScript|torch\.jit.*not.*support|TorchScript.*unsupported|torch\.jit\.script.*error|cannot be converted to TorchScript)",
        "compilation_error", "pytorch", ">=2.0", "linux", "true", 0.80, 0.86,
        "Python construct or PyTorch operation not supported in TorchScript. TorchScript supports a limited subset of Python.",
        [de("Use torch.jit.trace instead of torch.jit.script to avoid all errors",
            "Trace only captures one execution path; control flow, dynamic shapes, and data-dependent branches are lost", 0.75),
         de("Wrap unsupported code in try/except to catch scripting errors",
            "TorchScript compilation is ahead-of-time; try/except does not help with compilation failures", 0.82)],
        [wa("Refactor unsupported Python features to TorchScript-compatible equivalents", 0.88,
            "Replace set() with List, dict comprehensions with loops, f-strings with string concatenation"),
         wa("Use @torch.jit.ignore or @torch.jit.unused for non-exportable methods", 0.85,
            "@torch.jit.ignore\ndef debug_print(self, x): ...  # excluded from TorchScript, only callable in Python"),
         wa("Switch to torch.compile() (PyTorch 2.0+) instead of TorchScript", 0.82,
            "model = torch.compile(model)  # supports full Python, better performance than TorchScript")],
        python=">=3.9",
    ))

    canons.append(canon(
        "pytorch", "dataloader-bad-file-descriptor", "torch2-linux",
        "RuntimeError: DataLoader worker (pid(s) 12345) exited unexpectedly: OSError: [Errno 9] Bad file descriptor",
        r"(Bad file descriptor.*DataLoader|DataLoader worker.*exited.*OSError|Errno 9.*DataLoader|Bad file descriptor.*multiprocessing|DataLoader.*Broken pipe)",
        "multiprocessing_error", "pytorch", ">=2.0", "linux", "true", 0.80, 0.86,
        ...
        [de("Set num_workers=0 as a permanent fix",
            "Eliminates multiprocessing but makes data loading the bottleneck; training throughput drops significantly", 0.60),
         de("Increase ulimit for open files",
            "The issue is file handles not being picklable across process fork, not hitting the limit", 0.70)],
        [wa("Open files lazily in __getitem__ instead of __init__", 0.92,
            "Store file paths in __init__; open files in __getitem__: with open(self.paths[idx]) as f: data = f.read()"),
         wa("Use multiprocessing start method 'spawn' instead of 'fork'", 0.85,
            "torch.multiprocessing.set_start_method('spawn')  # avoids fork-related fd inheritance issues"),
         wa("Use persistent_workers=True to keep workers alive across epochs", 0.82,
            "DataLoader(dataset, num_workers=4, persistent_workers=True)  # workers keep their file handles open")],
        python=">=3.9",
    ))

    canons.append(canon(
        "pytorch", "state-dict-key-mismatch", "torch2-linux",
        ...
        r"(Missing key.*state_dict|Unexpected key.*state_dict|Error.*loading state_dict|size mismatch for.*state_dict|key.*mismatch.*load)",
        "serialization_error", "pytorch", ">=2.0", "linux", "true", 0.90, 0.91,
        ...
        [de("Use strict=False to ignore all mismatched keys",
            "strict=False silently skips missing/unexpected keys; model runs with random weights in those layers, producing garbage output", 0.82),
         de("Manually rename every key in the checkpoint",
            "Brittle and error-prone; breaks when the model changes. Fix the root cause instead.", 0.70)],
        [wa("Strip 'module.' prefix from DataParallel state dict", 0.92,
            "state_dict = {k.replace('module.', ''): v for k, v in checkpoint['state_dict'].items()}; model.load_state_dict(state_dict)"),
         wa("Save and load state_dict from the unwrapped model", 0.90,
            "torch.save(model.module.state_dict(), path)  # for DataParallel; or model.state_dict() for single GPU"),
         wa("Compare keys to diagnose the mismatch", 0.85,
    ...
    # =====================================================================
    canons.append(canon(
        "redis", "moved-ask-redirect", "redis7-linux",
        "MOVED 3999 127.0.0.1:7002",
        r"(MOVED \d+|ASK \d+|MOVED.*redirect|ASK.*redirect|slot.*moved|cluster.*redirect)",
        "cluster_error", "redis", ">=7.0", "linux", "true", 0.88, 0.90,
        ...
        [de("Retry the command on the same node hoping the slot migrates back",
            "MOVED is permanent; the slot belongs to a different node. Retrying on the same node always fails.", 0.85),
         de("Connect to a single cluster node and ignore redirects",
            "Single-node connection misses all keys not in that node's slots; most operations fail", 0.80)],
        [wa("Use a cluster-aware client library", 0.95,
            "redis-py: RedisCluster(host='...', port=7000); Jedis: JedisCluster; ioredis: new Cluster([{host, port}])"),
         wa("Handle MOVED by updating the slot-to-node mapping in the client", 0.88,
            "Cluster-aware clients do this automatically; raw clients must parse MOVED and reconnect to the target node"),
         wa("Handle ASK by sending ASKING command followed by the original command to the target node", 0.82,
            "ASK means slot is migrating; send ASKING + command to target node (temporary redirect, unlike MOVED)")],
    ))

    canons.append(canon(
        "redis", "crossslot-keys-error", "redis7-linux",
        "CROSSSLOT Keys in request don't hash to the same slot",
        r"(CROSSSLOT|keys.*don't hash.*same slot|cross.?slot|multiple keys.*different.*slots)",
        "cluster_error", "redis", ">=7.0", "linux", "true", 0.85, 0.89,
        ...
        [de("Disable cluster mode and use standalone Redis to avoid slot issues",
            "Loses all cluster benefits: high availability, horizontal scaling, automatic failover", 0.80),
    ...
    canons.append(canon(
        "redis", "loading-dataset-in-memory", "redis7-linux",
        "LOADING Redis is loading the dataset in memory",
        r"(LOADING.*dataset|Redis is loading|LOADING.*memory|loading the dataset in memory)",
        "startup_error", "redis", ">=7.0", "linux", "true", 0.90, 0.91,
        "Redis is still loading the RDB/AOF dataset into memory after a restart. All commands are rejected until loading completes.",
        [de("Kill and restart Redis again to speed up loading",
            "Restarting resets the loading process from zero; makes recovery take even longer", 0.90),
         de("Delete the dump.rdb/appendonly.aof file to skip loading",
            "Deletes all persisted data permanently; Redis starts empty", 0.88)],
        [wa("Wait for loading to complete; monitor progress with INFO", 0.92,
            "redis-cli INFO persistence  # check loading_eta_seconds and loading_loaded_perc"),
         wa("Configure application retry logic to handle transient LOADING state", 0.88,
            "Retry with exponential backoff on LOADING errors; typical load time: 1s per GB on SSD"),
         wa("Reduce dataset size or use faster storage for quicker startup", 0.80,
            "Use SSD instead of HDD; enable lazyfree-lazy-expire to reduce dataset size")],
    ))

    canons.append(canon(
        "redis", "busy-script-running", "redis7-linux",
        "BUSY Redis is busy running a script. You can only call SCRIPT KILL or SHUTDOWN NOSAVE.",
        r"(BUSY.*running a script|BUSY Redis|SCRIPT KILL|busy.*lua|script.*busy)",
        "scripting_error", "redis", ">=7.0", "linux", "true", 0.85, 0.88,
        "A Lua script or Redis function is running longer than lua-time-limit (default 5s), blocking all other commands.",
        [de("Use SHUTDOWN NOSAVE to stop the script",
            "SHUTDOWN terminates the entire Redis server; all connected clients lose their connection and unsaved data is lost", 0.90),
         de("Increase lua-time-limit to avoid the BUSY error",
            "Longer limit means longer blocking periods; all other clients are frozen while Lua executes", 0.72)],
        [wa("Use SCRIPT KILL to terminate the running script", 0.92,
            "redis-cli SCRIPT KILL  # only works if the script has NOT performed any writes yet"),
         wa("If script has written data, use SHUTDOWN NOSAVE only as last resort", 0.75,
            "SCRIPT KILL fails after writes; SHUTDOWN NOSAVE + restart loses unsaved changes. Consider the tradeoff."),
         wa("Fix the Lua script to avoid long execution", 0.88,
            "Break large operations into batches; use SCAN instead of KEYS; avoid O(n) operations on large sets")],
    ))

    canons.append(canon(
        "redis", "wrongtype-operation", "redis7-linux",
        "WRONGTYPE Operation against a key holding the wrong kind of value",
        r"(WRONGTYPE|wrong kind of value|WRONGTYPE Operation|wrong type.*operation)",
        "type_error", "redis", ">=7.0", "linux", "true", 0.92, 0.91,
        "Command used on a key that holds a different data type than expected. E.g., GET on a hash key, or LPUSH on a string key.",
        [de("Delete and recreate the key with the correct type",
            "Destroys existing data. Other parts of the application may depend on the current type.", 0.75),
         de("Use RENAME to work around the type check",
            "RENAME does not change the type; the renamed key still has the wrong type for the operation", 0.85)],
        [wa("Check key type before operating on it", 0.92,
            "redis-cli TYPE mykey  # returns string, list, set, zset, hash, stream"),
         wa("Fix the application code to use correct commands for the data type", 0.90,
            "String: GET/SET; Hash: HGET/HSET; List: LPUSH/LRANGE; Set: SADD/SMEMBERS; Sorted Set: ZADD/ZRANGE"),
         wa("Use separate key names or namespacing for different data types", 0.85,
            "user:123:name (string), user:123:profile (hash), user:123:orders (list)")],
    ))

    canons.append(canon(
        "redis", "auth-required", "redis7-linux",
        "NOAUTH Authentication required",
        r"(NOAUTH|Authentication required|NOAUTH Authentication|ERR Client sent AUTH.*no password|invalid password|AUTH.*failed)",
        "auth_error", "redis", ">=7.0", "linux", "true", 0.92, 0.92,
        "Redis requires authentication but the client did not send AUTH, or sent wrong credentials. Redis 7+ supports ACL users.",
        [de("Disable requirepass in redis.conf to remove authentication",
            "Removes all access control; any client can read/write/flush all data. Critical security risk.", 0.90),
         de("Hardcode the password in application source code",
            "Credentials in source code get committed to version control and exposed in logs", 0.80)],
        [wa("Configure the client library with the correct password", 0.95,
            "redis-py: Redis(host='...', password='secret'); or redis-cli -a secret"),
         wa("Use ACL users for fine-grained access control (Redis 7+)", 0.88,
            "ACL SETUSER appuser on >password ~app:* +get +set  # limit commands and key patterns"),
         wa("Store credentials in environment variables or a secrets manager", 0.85,
    ...
    # =====================================================================
    canons.append(canon(
        "cmake", "generator-not-found", "cmake3.28-linux",
        "CMake Error: Could not create named generator Ninja",
        r"(Could not create named generator|CMake Error.*generator.*not found|Generator.*is not available)",
        "configuration_error", "cmake", ">=3.20", "linux", "true", 0.90, 0.88,
        "Requested CMake generator (Ninja, Visual Studio, etc.) is not installed or not in PATH.",
        [de("Install a random build system hoping it provides the generator",
            "Only the specific requested generator is needed; installing Make does not provide Ninja.", 0.70),
         de("Downgrade CMake version to one that 'includes' the generator",
            "Generators are separate executables, not bundled with CMake. Version is irrelevant.", 0.75)],
        [wa("Install the required generator", 0.95,
            "sudo apt install ninja-build  # for Ninja generator"),
         wa("Use a different available generator", 0.88,
            "cmake -G 'Unix Makefiles' ..  # list available: cmake --help"),
         wa("Set PATH to include the generator binary location", 0.82,
            "export PATH=/opt/ninja/bin:$PATH && cmake -G Ninja ..")],
    ))
    ...
    canons.append(canon(
        ...
        [de("Set CMAKE_CXX_STANDARD_REQUIRED to OFF to ignore the error",
            "Silently falls back to an older standard; code using C++20 features will fail with cryptic template errors.", 0.85),
         de("Add -std=c++20 manually to CMAKE_CXX_FLAGS",
            "Bypasses CMake's compiler feature detection; causes duplicate or conflicting flags.", 0.72)],
        [wa("Upgrade the compiler to a version supporting the required standard", 0.92,
            "sudo apt install g++-12  # g++-12 supports C++20; g++-13+ for C++23"),
         wa("Specify the compiler version explicitly", 0.88,
            "cmake -DCMAKE_CXX_COMPILER=/usr/bin/g++-12 .."),
         wa("Lower CXX_STANDARD if the code does not actually use newer features", 0.80,
            "set(CMAKE_CXX_STANDARD 17)  # only if no C++20 features are used")],
    ))

    canons.append(canon(
        "cmake", "imported-target-not-found", "cmake3.28-linux",
        "CMake Error: Imported target 'Boost::filesystem' includes non-existent path",
        r"(Imported target.*non-existent|IMPORTED_LOCATION.*not set|imported target.*not found|target.*IMPORTED.*NOTFOUND)",
        "build_error", "cmake", ">=3.16", "linux", "true", 0.85, 0.88,
        "An imported target references paths that do not exist on disk. Library was moved, uninstalled, or partially installed.",
        [de("Create the missing path manually",
            "Manually creating library directories does not provide the actual libraries and headers.", 0.80),
         de("Remove CMakeCache.txt and assume reconfigure will find it",
            "If the library is not installed, reconfiguring still will not find it. Cache removal alone is not a fix.", 0.65)],
        [wa("Reinstall the dependency that provides the imported target", 0.92,
            "sudo apt install libboost-filesystem-dev  # provides Boost::filesystem target"),
         wa("Delete CMakeCache.txt AND install the missing library, then reconfigure", 0.90,
            "rm -rf build/ && sudo apt install libfoo-dev && cmake -B build"),
         wa("Set the package DIR variable to the correct install prefix", 0.82,
            "cmake -DBoost_DIR=/opt/boost/lib/cmake/Boost-1.83.0 ..")],
    ))

    canons.append(canon(
        "cmake", "add-subdirectory-not-found", "cmake3.28-linux",
        "CMake Error: The source directory /path/to/sub does not contain a CMakeLists.txt file.",
        r"(does not contain a CMakeLists\.txt|add_subdirectory.*not exist|source directory.*CMakeLists|CMakeLists\.txt.*missing.*subdirectory)",
        "configuration_error", "cmake", ">=3.16", "linux", "true", 0.92, 0.90,
        "add_subdirectory() points to a directory that lacks a CMakeLists.txt. Missing submodule, typo, or wrong path.",
        [de("Create an empty CMakeLists.txt in the subdirectory",
            "An empty file silences the error but the subdirectory's targets will not be defined, causing link errors later.", 0.82),
         de("Use EXCLUDE_FROM_ALL option to skip the directory",
            "EXCLUDE_FROM_ALL only skips default build; it does not skip configuration. CMake still needs the file to exist.", 0.78)],
        [wa("Initialize git submodules if the directory is a submodule", 0.95,
            "git submodule update --init --recursive"),
         wa("Check for typos in the directory path in CMakeLists.txt", 0.90,
            "# Verify: ls -la path/to/subdirectory/CMakeLists.txt"),
         wa("Guard with if(EXISTS) for optional subdirectories", 0.82,
            "if(EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/optional_sub/CMakeLists.txt)\n  add_subdirectory(optional_sub)\nendif()")],
    ))

    canons.append(canon(
        "cmake", "policy-cmp-warning", "cmake3.28-linux",
        "CMake Warning (dev): Policy CMP0077 is not set: option() honors normal variables.",
        r"(Policy CMP\d+.*not set|CMake.*policy.*CMP\d+|cmake_policy.*OLD|deprecated.*policy|CMP\d+.*NEW.*OLD)",
        "deprecation_warning", "cmake", ">=3.16", "linux", "true", 0.88, 0.85,
        ...
        [de("Set cmake_policy(SET CMPxxxx OLD) to suppress the warning",
            "OLD behavior is deprecated and will be removed in future CMake versions. This is a ticking time bomb.", 0.80),
         de("Pin cmake_minimum_required to an old version to avoid the policy",
            "Locks entire project to old CMake features and behavior, not just the single policy.", 0.85)],
        [wa("Update code to comply with the NEW policy behavior", 0.92,
            "cmake_policy(SET CMP0077 NEW)  # then update code to match new behavior"),
         wa("Set cmake_minimum_required to a version where the policy defaults to NEW", 0.88,
            "cmake_minimum_required(VERSION 3.24)  # CMP0077 defaults to NEW since 3.13"),
         wa("Use cmake_policy(SET CMPxxxx NEW) explicitly in the relevant scope", 0.85,
    ...
    # =====================================================================
    canons.append(canon(
        "embedded", "flash-write-failed", "stm32-arm-linux",
        "Error: flash write failed at address 0x08000000, expected 0xFF read back 0x00",
        r"(flash write failed|Error.*flash.*program|write.*failed.*address.*0x|flash.*verification.*failed|FLASH_SR.*WRPERR)",
        "flash_error", "openocd", ">=0.11", "linux", "true", 0.82, 0.88,
        "Flash programming fails. Write-protection enabled, flash not erased, or voltage too low for programming.",
        [de("Retry flashing repeatedly without erasing first",
            "Flash can only be written 1->0. If not erased (all 0xFF), writes silently corrupt. Always erase before write.", 0.85),
         de("Increase SWD clock speed for faster flash writes",
            "Flash programming speed is limited by internal charge pump, not SWD clock. Faster clock increases errors.", 0.72)],
        [wa("Erase flash before writing", 0.95,
            "openocd: flash erase_sector 0 0 last  # or use mass_erase"),
         wa("Remove flash write protection via option bytes", 0.88,
            "STM32CubeProgrammer -> Option Bytes -> Write Protection -> Uncheck all sectors"),
         wa("Check supply voltage is above minimum for flash programming", 0.80,
            "STM32F4 needs >= 2.7V for word programming. Brown-out during flash causes corruption.")],
        arch="arm",
    ))

    canons.append(canon(
        "embedded", "stack-overflow-detected", "stm32-arm-linux",
        "FreeRTOS: Stack overflow detected in task 'MainTask' (high water mark: 0)",
        r"(stack overflow|vApplicationStackOverflowHook|uxTaskGetStackHighWaterMark.*0|configCHECK_FOR_STACK_OVERFLOW|stack.*corrupt)",
        "runtime_error", "freertos", ">=10.0", "cross-platform", "true", 0.85, 0.90,
        "RTOS task stack overflowed. Stack too small for call depth, large local arrays, or recursive calls.",
        [de("Double all task stack sizes blindly",
            "Wastes precious SRAM. Identify the offending task and size its stack based on actual usage.", 0.72),
         de("Disable stack overflow checking to hide the error",
            "Stack corruption continues silently, corrupting adjacent task stacks or heap, causing random crashes.", 0.90)],
        [wa("Check high water mark and increase offending task stack", 0.92,
            "UBaseType_t hwm = uxTaskGetStackHighWaterMark(NULL); // words remaining. Increase stack by deficit + margin."),
         wa("Move large local arrays to static or heap allocation", 0.88,
            "static uint8_t buf[1024]; // instead of stack-allocated. Or use pvPortMalloc()."),
         wa("Enable configCHECK_FOR_STACK_OVERFLOW=2 for early detection", 0.85,
            "#define configCHECK_FOR_STACK_OVERFLOW 2  // checks watermark on every context switch")],
        arch="arm",
    ))

    canons.append(canon(
        "embedded", "watchdog-timeout-reset", "stm32-arm-linux",
        "IWDG reset detected: system restarted by independent watchdog timeout",
        r"(watchdog.*reset|IWDG.*timeout|WWDG.*reset|wdt.*expired|watchdog.*triggered|RCC_CSR.*IWDG)",
        "runtime_error", "stm32", ">=F1", "cross-platform", "true", 0.80, 0.85,
        "Watchdog timer reset the MCU because firmware did not refresh it in time. Infinite loop, deadlock, or blocking delay.",
        [de("Disable the watchdog entirely",
            "Watchdog exists for safety/reliability. Disabling it hides the root cause; system hangs without recovery.", 0.88),
         de("Increase watchdog timeout to maximum value",
            "Masks the problem temporarily. If code hangs for even longer, the crash returns.", 0.70)],
        [wa("Check reset source register to confirm watchdog reset", 0.88,
            "if (RCC->CSR & RCC_CSR_IWDGRSTF) { /* IWDG caused reset */ } RCC->CSR |= RCC_CSR_RMVF;"),
         wa("Add watchdog refresh in all long-running loops and blocking waits", 0.90,
            "while (wait_for_sensor()) { HAL_IWDG_Refresh(&hiwdg); }  // prevent timeout during legitimate waits"),
         wa("Use a dedicated low-priority RTOS task to feed the watchdog", 0.82,
            "If lowest-priority task runs, all higher-priority tasks are not starving the system.")],
        arch="arm",
    ))

    canons.append(canon(
        "embedded", "spi-transfer-timeout", "stm32-arm-linux",
        "HAL_SPI_Transmit timeout: SPI TXE flag not set within timeout period",
        r"(SPI.*timeout|HAL_SPI.*Timeout|SPI.*TXE.*not set|SPI.*RXNE.*timeout|SPI.*BSY.*stuck|HAL_TIMEOUT)",
        "communication_error", "stm32", ">=F1", "cross-platform", "true", 0.85, 0.88,
        "SPI transfer times out waiting for TXE/RXNE flags. Clock not running, wrong mode, or NSS misconfiguration.",
        [de("Increase HAL timeout value",
            "If SPI clock is not running, no timeout value will help. The peripheral itself is misconfigured.", 0.82),
         de("Switch from hardware NSS to software NSS without changing code",
            "Hardware vs software NSS requires different GPIO and CR register configuration; just toggling the mode breaks CS.", 0.75)],
        [wa("Verify SPI clock is enabled in RCC", 0.92,
            "__HAL_RCC_SPI1_CLK_ENABLE();  // must be called before any SPI init"),
         wa("Check SPI mode (CPOL/CPHA) matches the slave device", 0.90,
            "Check slave datasheet for clock polarity and phase. Mode 0 (CPOL=0,CPHA=0) is most common."),
         wa("Verify GPIO alternate function mapping for SCK, MOSI, MISO pins", 0.85,
            "GPIO_InitStruct.Alternate = GPIO_AF5_SPI1;  // AF mapping varies by STM32 family and pin")],
        arch="arm",
    ))

    canons.append(canon(
        "embedded", "linker-section-overflow", "stm32-arm-linux",
        "arm-none-eabi-ld: region 'FLASH' overflowed by 4128 bytes",
        r"(region.*overflowed by|will not fit in region|section.*is too large|ld.*FLASH.*overflow|ld.*RAM.*overflow|output.*exceeds.*region)",
        "linker_error", "gcc-arm", ">=10.0", "cross-platform", "true", 0.85, 0.88,
        "Compiled code or data exceeds the microcontroller's memory region. Too much code for FLASH or too much data for RAM.",
        [de("Increase memory region size in linker script beyond physical limits",
            "Linker script sizes must match physical memory. Exceeding them causes writes to unmapped memory and hard faults.", 0.92),
         de("Disable all compiler warnings to reduce binary size",
            "Warnings are compile-time text, not code. Disabling them has zero effect on binary size.", 0.95)],
        [wa("Compile with -Os to optimize for size", 0.90,
            "set(CMAKE_C_FLAGS_RELEASE \"-Os\")  # or -Og for debug with some size savings"),
         wa("Enable link-time optimization (LTO) to remove unused code", 0.88,
            "add_compile_options(-flto) and add_link_options(-flto)"),
         wa("Analyze section sizes and remove large unused modules", 0.85,
    ...
    # =====================================================================
    canons.append(canon(
        "opencv", "dnn-backend-not-supported", "cv4.9-linux",
        "cv2.error: DNN module was not built with CUDA backend; switched to CPU",
        r"(DNN.*not built with|backend.*not (available|supported)|setPreferableBackend.*failed|CUDA.*backend.*not available|DNN_BACKEND_CUDA.*error)",
        "configuration_error", "opencv", ">=4.5", "linux", "true", 0.85, 0.88,
        "OpenCV DNN module does not have the requested backend (CUDA, OpenVINO). Built without that support.",
        [de("Install CUDA toolkit expecting opencv-python pip package to use it",
            "pip opencv-python is built without CUDA. You need opencv-contrib-python built from source with CUDA flags.", 0.88),
         de("Set backend to CUDA in code without checking availability",
            "Falls back to CPU silently or throws error. Always check getLayerTypes() or getBuildInformation().", 0.72)],
        [wa("Build OpenCV from source with CUDA flags", 0.88,
            "cmake -DWITH_CUDA=ON -DCUDA_ARCH_BIN=8.6 -DOPENCV_DNN_CUDA=ON .."),
         wa("Use CPU backend which is always available", 0.92,
            "net.setPreferableBackend(cv2.dnn.DNN_BACKEND_OPENCV)\nnet.setPreferableTarget(cv2.dnn.DNN_TARGET_CPU)"),
         wa("Check build info to see which backends are available", 0.85,
            "print(cv2.getBuildInformation())  # look for CUDA, OpenVINO under DNN section")],
    ))

    canons.append(canon(
        "opencv", "codec-not-found", "cv4.9-linux",
        "OpenCV: FFMPEG: tag 0x34363248/'H264' is not supported with codec id 27",
        r"(tag.*not supported with codec|fourcc.*not (found|supported)|FFMPEG.*codec.*error|VideoWriter.*could not find.*codec|CAP_FFMPEG.*codec)",
        "codec_error", "opencv", ">=4.5", "linux", "true", 0.82, 0.85,
        "VideoWriter cannot find the requested codec fourcc. Codec not installed or not compiled into FFmpeg backend.",
        [de("Use raw/uncompressed fourcc to avoid codec issues",
            "Uncompressed video produces enormous files (1080p@30fps ~ 5.6 GB/min). Not a practical solution.", 0.68),
         de("Install a random codec pack",
            "OpenCV uses its bundled FFmpeg or system FFmpeg. Random codec packs do not affect it.", 0.80)],
        [wa("Use a widely-supported codec fourcc", 0.92,
            "fourcc = cv2.VideoWriter_fourcc(*'mp4v')  # MPEG-4 is nearly universally available"),
         wa("Install FFmpeg with the required codec", 0.88,
            "sudo apt install ffmpeg libx264-dev && pip install --force-reinstall opencv-python"),
         wa("Check available backends and codecs", 0.82,
    ...
    canons.append(canon(
        ...
        [de("Wrap imread in try/except to catch the error",
            "imread does not raise exceptions on failure; it returns None. try/except catches nothing.", 0.92),
         de("Convert path to raw string assuming backslash issue on Linux",
            "Linux uses forward slashes. Raw strings only matter on Windows with backslash paths.", 0.78)],
        [wa("Check return value of imread before using the image", 0.95,
            "img = cv2.imread(path)\nif img is None:\n    raise FileNotFoundError(f'imread failed: {path}')"),
         wa("Verify the file exists and is a supported format", 0.90,
            "import os; assert os.path.isfile(path), f'File not found: {path}'  # also check file is not 0 bytes"),
         wa("Use absolute path to avoid working directory issues", 0.85,
            "img = cv2.imread(os.path.abspath(path))  # relative paths depend on cwd, not script location")],
    ))

    canons.append(canon(
        "opencv", "cuda-module-not-compiled", "cv4.9-linux",
        "cv2.error: OpenCV was built without CUDA support; cv2.cuda is not available",
        r"(built without CUDA|cv2\.cuda.*not available|CUDA.*not (compiled|enabled|built)|cuda.*module.*not found|No CUDA.*support)",
        "configuration_error", "opencv", ">=4.5", "linux", "true", 0.82, 0.90,
        "OpenCV cv2.cuda module is not available because the installed build was compiled without CUDA support.",
        [de("pip install opencv-contrib-python expecting CUDA support",
            "PyPI wheels are not built with CUDA. Even opencv-contrib-python from pip has no CUDA.", 0.90),
         de("Install CUDA toolkit after opencv-python hoping it activates CUDA",
            "OpenCV must be compiled with CUDA at build time. Installing CUDA later has no effect on existing OpenCV.", 0.88)],
        [wa("Build OpenCV from source with CUDA enabled", 0.90,
            "cmake -DWITH_CUDA=ON -DCUDA_ARCH_BIN=8.6 -DOPENCV_EXTRA_MODULES_PATH=../opencv_contrib/modules .."),
         wa("Use pre-built CUDA-enabled OpenCV packages", 0.85,
            "conda install -c conda-forge opencv  # some conda builds include CUDA"),
         wa("Use CPU equivalents of CUDA functions for portability", 0.88,
            "# Instead of cv2.cuda.GpuMat, use regular cv2.Mat operations which run on CPU")],
    ))

    canons.append(canon(
        "opencv", "calibration-insufficient-points", "cv4.9-linux",
        "cv2.error: (-28:Insufficient number of valid points) in function 'calibrateCamera'",
        r"(Insufficient.*valid points|calibrateCamera.*failed|not enough.*corners|findChessboardCorners.*returned False|insufficient.*calibration)",
        "calibration_error", "opencv", ">=4.5", "linux", "true", 0.85, 0.88,
        ...
        [de("Reduce the number of required corners to match what was detected",
            "Specifying fewer corners than the actual pattern causes wrong correspondences and bad calibration.", 0.85),
         de("Use low-resolution images to make corner detection faster",
            "Lower resolution reduces corner detection accuracy and subpixel refinement quality.", 0.72)],
        [wa("Capture more calibration images (at least 15-20) at varied angles", 0.92,
            "Tilt board at 30-45 degrees. Cover all regions of the frame. Vary distance from camera."),
         wa("Verify findChessboardCorners succeeds before adding to calibration set", 0.90,
            "ret, corners = cv2.findChessboardCorners(gray, (9, 6))\nif ret:\n    objpoints.append(objp)\n    imgpoints.append(corners)"),
         wa("Improve pattern detection with preprocessing", 0.85,
